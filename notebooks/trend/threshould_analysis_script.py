...
def calibration_analysis(model_type, RICH_CUT_list, MASS_CUT_list):
...
def bayes_factor(RICH_CUT_list, MASS_CUT_list):
    bef_list = []
...
def calibration_analysis_table(paper_trend):
...
def comparison_plot(calib1, calib2, MASS_CUT_list):    
...
def bayes_factor_plot(RICH_CUT_list, MASS_CUT_list):
    plt.figure(figsize=(10,6))

    bf_table = bayes_factor(RICH_CUT_list, MASS_CUT_list)
    
    bef_list = [bf_table.iloc[i:i+7] for i in range(0, len(bf_table), 7)]
    
    for i in range(0,5): 
        
        plt.plot(bef_list[i]["Min_Richness"], bef_list[i]["ln(BF)"], label = f'{MASS_CUT_list[i]:.2}', ls = '-.', linewidth = 1.0, marker = 'o')
        plt.yscale("log")
