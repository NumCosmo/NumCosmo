...
def generate_halo_cluster(cosmo,dist,HALO_LENGTH=200, CLUSTER_LENGTH=100, RA_interval=[-10,10], DEC_interval=[-10,10], z_interval=[0.2,0.5],seed=74682):
# Lets fix the numpy seed to get reproducible results
    ...
    halos['R200'] = ((3*(halo_mass))/(4*np.pi* 200*(rho_crit(cosmo, halos['z']))))**(1/3)
    clusters['R200'] = ((3*(cluster_mass))/(4*np.pi* 200*(rho_crit(cosmo, clusters['z']))))**(1/3)
...
def sharing_galaxies(halos,halo_galaxies,cluster_galaxies, cosmo, halo_properties, detections_properties, obj):
