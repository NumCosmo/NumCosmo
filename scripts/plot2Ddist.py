...
def contour_enclosing(
    x, y, fractions, xgrid, ygrid, zvals, axes, nstart=200, *args, **kwargs
):
...
def plot2Ddist(
    ...
    * marginal histograms for each variable
    ...
    if plotcontours:
        ...
        xgrid, ygrid = numpy.mgrid[
            xlim[0] : xlim[1] : contourNGrid * 1j, ylim[0] : ylim[1] : contourNGrid * 1j
        ]
        # xgrid, ygrid = numpy.mgrid[min(x):max(x):contourNGrid * 1j,
        # min(y):max(y):contourNGrid * 1j]
        zvals = numpy.array(gkde.evaluate([xgrid.flatten(), ygrid.flatten()])).reshape(
            xgrid.shape
        )
        contours = contour_enclosing(
            x, y, contourFractions, xgrid, ygrid, zvals, ax1, **style
        )
