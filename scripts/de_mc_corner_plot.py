...
while len(sys.argv) > 2:
    ...
    for i in pindex:
        ...
        epsilon = 1.0e-7
        col_min = np.percentile(
            col, epsilon
        )  # range is defined computing the 6\sigma confidence interval
    ...
    col_data_a: np.ndarray = np.asarray(col_data)
    col_data_a = np.matrix.transpose(np.asarray(col_data_a))
    col_data_a = col_data[~np.all(np.abs(col_data_a) < 1e-6, axis=1)]
    print(col_data_a.shape)
...
for file in files:
    ...
    if file["high_prec"] == 0:
        fig = corner.corner(
            cols, labels=col_symbols, range=params_range, show_titles=True
        )
    else:
        fig = plotCorner(
            cols, bins=50, labels=col_symbols, label_size=20,
            range=params_range
        )
