...
@pytest.fixture(name="wspline_model", params=[3, 4, 6, 12])
def fixture_wspline_model(request) -> Nc.HICosmoDEWSpline:
    """Returns a DEWSpline model with preset knots and values."""
    return Nc.HICosmoDEWSpline.new(nknots=request.param, z_f=2.0)


def test_wspline_knots(wspline_model: Nc.HICosmoDEWSpline) -> None:
    """Test that knots are set and retrieved correctly."""

    alpha = wspline_model.get_alpha()
...
def test_wspline_eval_w(wspline_model: Nc.HICosmoDEWSpline) -> None:
...
def test_wspline_mean_kappa(wspline_model: Nc.HICosmoDEWSpline) -> None:
    """Test evaluation of mean kappa at knot points and in between."""
    alpha = wspline_model.get_alpha()
    w_len = alpha.len()
    w_array = np.random.uniform(-1.5, -0.5, size=w_len).astype(float)

    kappa_mean = wspline_model.mean_kappa()
...
def test_wspline_mean_kappa_mset_func(wspline_model: Nc.HICosmoDEWSpline) -> None:
    """Test evaluation of mean kappa at knot points and in between."""
    alpha = wspline_model.get_alpha()
    w_len = alpha.len()
    w_array = np.random.uniform(-1.5, -0.5, size=w_len).astype(float)
    ...
    mset = Ncm.MSet.new_array([wspline_model])
