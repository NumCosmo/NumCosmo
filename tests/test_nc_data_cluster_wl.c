...
test_nc_data_cluster_wl_gen_gauss (TestNcDataClusterWL *test, gconstpointer pdata)
{
  ...
  if (NC_IS_GALAXY_SD_OBS_REDSHIFT_GAUSS (test->galaxy_redshift))
  {
    for (i = 0; i < nrows; i++)
    {
      ...
      nc_galaxy_sd_shape_gauss_gen (NC_GALAXY_SD_SHAPE_GAUSS (test->galaxy_shape), test->mset, s_data, 0.1, NC_GALAXY_WL_OBS_COORD_EUCLIDEAN, rng);
...
test_nc_data_cluster_wl_gen_obs_gauss (TestNcDataClusterWL *test, gconstpointer pdata)
{
  ...
  for (i = 0; i < ngals; i++)
  {
    ...
    const gdouble sigma_obs     = nc_galaxy_wl_obs_get (obs, NC_GALAXY_SD_SHAPE_GAUSS_COL_STD_NOISE, i);
    const gdouble var_obs       = sigma_obs * sigma_obs;
    const gdouble sigma_int     = ncm_model_orig_param_get (NCM_MODEL (test->galaxy_shape), NC_GALAXY_SD_SHAPE_GAUSS_SIGMA);
    const gdouble var_int       = sigma_int * sigma_int;
    const gdouble e_rms_1       = sqrt (var_int + var_obs);
    const gdouble e_rms_2       = sqrt (var_int + var_obs);
    gdouble theta, phi;
