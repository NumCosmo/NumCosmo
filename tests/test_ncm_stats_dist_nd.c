...
{
  test->dim   = g_test_rand_int_range (2, 8);
  test->dnd   = NCM_STATS_DIST_ND (ncm_stats_dist_nd_kde_gauss_new (test->dim));
  test->nfail = 0;
}
...
test_ncm_stats_dist_nd_gauss_dens_est (TestNcmStatsDistNd *test, gconstpointer pdata)
{
  ...
  const guint np                 = 1000 * test->dim * g_test_rand_int_range (1, 5);
...
test_ncm_stats_dist_nd_gauss_dens_interp (TestNcmStatsDistNd *test, gconstpointer pdata)
{
  ...
  ncm_stats_dist_nd_prepare_interp (test->dnd, m2lnp_v, TRUE);
  ...
  for (i = 0; i < ntests; i++)
  {
    ...
    if (fabs (p_s / exp (-0.5 * m2lnL) - 1.0) > 0.5)
     ...
     /*ncm_vector_log_vals (y, "Y: ", "% 12.5g", TRUE); */
     /*printf ("# EVAL: % 22.15g % 22.15g % 22.15f\n", -2.0 * log (p_s),  m2lnL, -2.0 * log (p_s) - m2lnL); */
  }
...
test_ncm_stats_dist_nd_gauss_dens_interp_unormalized (TestNcmStatsDistNd *test, gconstpointer pdata)
{
  ...
  ncm_stats_dist_nd_prepare_interp (test->dnd, m2lnp_v, FALSE);
  ...
  for (i = 0; i < ntests; i++)
  {
    ...
    cmp = exp (-0.5 * (m2lnp_s - m2lnL)) - 1.0;
    
    if (fabs (cmp) > 0.75)
      ntests_fail++;
    
     /*ncm_vector_log_vals (y, "Y: ", "% 12.5g", TRUE); */
     /*printf ("# EVAL: % 22.15g % 22.15g % 22.15f % 22.15f\n", m2lnp_s, m2lnL, exp (-0.5 * (m2lnp_s - m2lnL)), cmp);*/
  }
...
test_ncm_stats_dist_nd_gauss_sampling (TestNcmStatsDistNd *test, gconstpointer pdata)
{
  ...
  NcmMSet *mset                  = ncm_mset_new (NCM_MODEL (model_mvnd), NULL);
  const guint np                 = 1000 * test->dim * g_test_rand_int_range (1, 5);
  const guint ntests             = 10000 * g_test_rand_int_range (1, 5);
