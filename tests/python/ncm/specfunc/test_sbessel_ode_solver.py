...
class TestSBesselOperators:
    ...
    @pytest.mark.parametrize(
        "method",
        [
            Ncm.SBesselOdeSolverIVPMethod.STANDARD,
            Ncm.SBesselOdeSolverIVPMethod.FROBENIUS,
        ],
    )
    def test_spherical_bessel_integration_ivp(
        self, l_val: int, method: Ncm.SBesselOdeSolverIVPMethod
    ) -> None:
        """Compare the BVP and IVP forms of the Green-identity integral.

        Both solvers use the polynomial equation
        ``x^2 u'' + (x^2 - l(l+1)) u = f(x)``. The BVP has homogeneous
        values at ``a`` and ``b``; the IVP starts at ``a`` with ``u(a)=u'(a)=0``.
        Their Green boundary terms must therefore give the same integral on
        ``[a, b]``.
        """
        N = 128
        # Keep the interval beyond the largest turning point. A forward IVP
        # started deep in the evanescent region is intrinsically ill
        # conditioned: tiny errors excite its growing homogeneous solution.
        a, b = 30.0, 50.0
        h = (b - a) / 2.0
        m = (a + b) / 2.0

        # x^2 = (m^2 + h^2 / 6) C_0^(2) + (m h / 2) C_1^(2)
        #       + (h^2 / 12) C_2^(2).
        rhs_np = np.zeros(N)
        rhs_np[2] = m * m + h * h / 6.0
        rhs_np[3] = m * h / 2.0
        rhs_np[4] = h * h / 12.0

        bvp_solver = Ncm.SBesselOdeSolver.new()
        ...
        ivp_solver = Ncm.SBesselOdeSolverIVP.new()
        ivp_solver.set_method(method)
        ivp_solver.set_reltol(1.0e-11)
        ivp_solver.set_abstol(1.0e-13)

        def forcing(_user_data: object, x: float) -> float:
            return x * x

        u_b, du_b = ivp_solver.solve(forcing, a, b, l_val, None)

        def ivp_boundary_term(
            x: float, j_l: float, dj_l: float, u: float, du: float
        ) -> float:
            return x * j_l * du - (j_l + x * dj_l) * u

        ivp_integral = ivp_boundary_term(b, j_b, dj_b, u_b, du_b)
        direct_integral, _error = quad(
            lambda x: x * spherical_jn(l_val, x),
            a,
            b,
            epsabs=1.0e-12,
            epsrel=1.0e-12,
        ...
        assert_allclose(bvp_integral, direct_integral, rtol=1.0e-9, atol=1.0e-12)
        assert_allclose(ivp_integral, direct_integral, rtol=1.0e-8, atol=1.0e-10)
        assert_allclose(ivp_integral, bvp_integral, rtol=1.0e-8, atol=1.0e-10)
