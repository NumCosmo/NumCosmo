...
static void test_nc_galaxy_sd_obs_redshift_gauss_eval_pzp (TestNcGalaxySDObsRedshift *test, gconstpointer pdata);
static void test_nc_galaxy_sd_obs_redshift_gauss_eval_pz_given_zp (TestNcGalaxySDObsRedshift *test, gconstpointer pdata);
...
main (gint argc, gchar *argv[])
{
  ...
  g_test_add ("/nc/galaxy_sd_obs_redshift/gauss/eval_pzp", TestNcGalaxySDObsRedshift, NULL,
              &test_nc_galaxy_sd_obs_redshift_gauss_new,
              &test_nc_galaxy_sd_obs_redshift_gauss_eval_pzp,
              &test_nc_galaxy_sd_obs_redshift_free);

  g_test_add ("/nc/galaxy_sd_obs_redshift/gauss/eval_pz_given_zp", TestNcGalaxySDObsRedshift, NULL,
              &test_nc_galaxy_sd_obs_redshift_gauss_new,
              &test_nc_galaxy_sd_obs_redshift_gauss_eval_pz_given_zp,
              &test_nc_galaxy_sd_obs_redshift_free);
...
test_nc_galaxy_sd_obs_redshift_spec_gen (TestNcGalaxySDObsRedshift *test, gconstpointer pdata)
{
  ...
  for (i = 0; i < nruns; i++)
  {
    ...
    NcmRNG *rng3                     = ncm_rng_seeded_new (NULL, seed);
    NcGalaxySDObsRedshiftData *data2 = nc_galaxy_sd_obs_redshift_data_new (test->gsdor);
    NcGalaxySDObsRedshiftData *data3 = nc_galaxy_sd_obs_redshift_data_new (test->gsdor);

    nc_galaxy_sd_obs_redshift_spec_gen (NC_GALAXY_SD_OBS_REDSHIFT_SPEC (test->gsdor), test->mset, data2, rng2);
...
test_nc_galaxy_sd_obs_redshift_gauss_gen (TestNcGalaxySDObsRedshift *test, gconstpointer pdata)
{
  ...
  for (i = 0; i < nruns; i++)
  {
    ...
    for (j = 0; j < ndata; j++)
    {
      ...
      nc_galaxy_sd_obs_redshift_gauss_data_get (NC_GALAXY_SD_OBS_REDSHIFT_GAUSS (test->gsdor), data, &zp, &sigma0, &sigma_z);

      g_assert_cmpfloat (zp, >=, test->z_min);
      g_assert_cmpfloat (zp, <=, test->z_max);

      ncm_stats_vec_set (pos_sample, 0, data->z);
      ncm_stats_vec_set (pos_sample, 1, zp);
  ...
  for (i = 0; i < nruns; i++)
  {
    ...
    for (j = 0; j < ndata; j++)
    {
      ...
      if (result)
        g_assert_true ((zp >= test->z_min) && (zp <= test->z_max));
      else
        g_assert_true ((zp <= test->z_min) || (zp >= test->z_max));
  ...
  for (i = 0; i < nruns; i++)
  {
    ...
    NcGalaxySDObsRedshiftData *data2 = nc_galaxy_sd_obs_redshift_data_new (test->gsdor);
    NcGalaxySDObsRedshiftData *data3 = nc_galaxy_sd_obs_redshift_data_new (test->gsdor);
    gboolean result2                 = nc_galaxy_sd_obs_redshift_spec_gen1 (NC_GALAXY_SD_OBS_REDSHIFT_SPEC (test->gsdor), test->mset, data2, rng2);
    gboolean result3                 = nc_galaxy_sd_obs_redshift_gen1 (test->gsdor, data3, rng3);
    gdouble zp2, sigma02, sigma_z2;

    nc_galaxy_sd_obs_redshift_gauss_data_get (NC_GALAXY_SD_OBS_REDSHIFT_GAUSS (test->gsdor), data2, &zp, &sigma0, &sigma_z);
    nc_galaxy_sd_obs_redshift_gauss_data_get (NC_GALAXY_SD_OBS_REDSHIFT_GAUSS (test->gsdor), data3, &zp2, &sigma02, &sigma_z2);
...
test_nc_galaxy_sd_obs_redshift_gauss_integ (TestNcGalaxySDObsRedshift *test, gconstpointer pdata)
{
  NcmRNG *rng                                  = ncm_rng_seeded_new (NULL, g_test_rand_int ());
  NcGalaxySDObsRedshiftData *data              = nc_galaxy_sd_obs_redshift_data_new (test->gsdor);
  NcGalaxySDObsRedshiftIntegrand *integrand    = nc_galaxy_sd_obs_redshift_integ (test->gsdor, FALSE);
  NcGalaxySDObsRedshiftIntegrand *ln_integrand = nc_galaxy_sd_obs_redshift_integ (test->gsdor, TRUE);
  ...
  nc_galaxy_sd_obs_redshift_integrand_prepare (ln_integrand, mset);
  ...
  for (i = 0; i < nruns; i++)
  {
    ...
    const gdouble lognorm    = ncm_util_log_gaussian_integral (test->z_min, test->z_max, z, sigmaz, &sign);
    const gdouble ln_int_z   = nc_galaxy_sd_true_redshift_ln_integ (gsdtr, z);
    const gdouble ln_int_zp  = (-0.5 * gsl_pow_2 ((zp - z) / sigmaz) - lognorm) - log (norm);
    const gdouble ln_control = ln_int_z + ln_int_zp;
    const gdouble ln_res     = nc_galaxy_sd_obs_redshift_integrand_eval (ln_integrand, z, data);
    ...
    ncm_assert_cmpdouble_e (ln_control, ==, ln_res, 1.0e-10, 0.0);
    ncm_assert_cmpdouble_e (log (res), ==, ln_res, 1.0e-10, 0.0);
  }
  ...
  for (i = 0; i < nruns; i++)
  {
    /* High z causes log(res) to diverge from ln_res */
    const gdouble z          = g_test_rand_double_range (0.0, 1.0);
    const gdouble norm       = sqrt (2.0 * M_PI) * sigma0;
    const gdouble ln_control = -0.5 * gsl_pow_2 ((zp - z) / sigma0) - log (norm);
    const gdouble ln_res     = nc_galaxy_sd_obs_redshift_integrand_eval (ln_integrand, z, data);
    const gdouble control    = exp (-0.5 * gsl_pow_2 ((zp - z) / sigma0)) / norm;
    const gdouble res        = nc_galaxy_sd_obs_redshift_integrand_eval (integrand, z, data);
...
test_nc_galaxy_sd_obs_redshift_spec_integ (TestNcGalaxySDObsRedshift *test, gconstpointer pdata)
{
  ...
  for (i = 0; i < nruns; i++)
  {
    ...
    const gdouble ln_int_true_z = nc_galaxy_sd_true_redshift_ln_integ (gsdtr, z);
    const gdouble int_obs_z     = nc_galaxy_sd_obs_redshift_integrand_eval (integrand, z, data);
    const gdouble ln_int_obs_z  = nc_galaxy_sd_obs_redshift_integrand_eval (ln_integrand, z, data);

    ncm_assert_cmpdouble_e (int_obs_z, ==, int_true_z, 1.0e-10, 0.0);
    ncm_assert_cmpdouble_e (ln_int_obs_z, ==, ln_int_true_z, 1.0e-10, 0.0);
    /* High z causes log(int_obs_z) to diverge from ln_int_obs_z */
    ncm_assert_cmpdouble_e (log (int_obs_z), ==, ln_int_obs_z, 1.0e-6, 0.0);
  }

  nc_galaxy_sd_obs_redshift_data_unref (data);
  nc_galaxy_sd_obs_redshift_integrand_free (integrand);
  nc_galaxy_sd_obs_redshift_integrand_free (ln_integrand);
  ncm_mset_free (mset);
  ncm_rng_clear (&rng);
}
...
test_nc_galaxy_sd_obs_redshift_pz_integ (TestNcGalaxySDObsRedshift *test, gconstpointer pdata)
{
  ...
  for (i = 0; i < nruns; i++)
  {
    ...
    const gdouble f        = exp (-0.5 * gsl_pow_2 ((z - z_avg) / z_sd)) / (sqrt (2.0 * M_PI) * z_sd);
    const gdouble ln_f     = (-0.5 * gsl_pow_2 ((z - z_avg) / z_sd)) - log (sqrt (2.0 * M_PI) * z_sd);
    const gdouble integ    = nc_galaxy_sd_obs_redshift_integrand_eval (integrand, z, data);
    const gdouble ln_integ = nc_galaxy_sd_obs_redshift_integrand_eval (ln_integrand, z, data);

    ncm_assert_cmpdouble_e (f, ==, integ, 1.0e-6, 0.0);
    ncm_assert_cmpdouble_e (ln_f, ==, ln_integ, 1.0e-6, 0.0);
    ncm_assert_cmpdouble_e (log (integ), ==, ln_integ, 1.0e-6, 0.0);
  }
...
test_nc_galaxy_sd_obs_redshift_spec_lim (TestNcGalaxySDObsRedshift *test, gconstpointer pdata)
{
  ...
  g_assert_cmpfloat (z_min, ==, int_z_min);
  g_assert_cmpfloat (z_max, ==, int_z_max);
...
test_nc_galaxy_sd_obs_redshift_gauss_lim (TestNcGalaxySDObsRedshift *test, gconstpointer pdata)
{
  ...
  nc_galaxy_sd_obs_redshift_gauss_get_zp_lim (NC_GALAXY_SD_OBS_REDSHIFT_GAUSS (test->gsdor), &z_min, &z_max);
...
static void
test_nc_galaxy_sd_obs_redshift_gauss_eval_pzp (TestNcGalaxySDObsRedshift *test, gconstpointer pdata)
{
  NcGalaxySDObsRedshiftGauss *gsdorgauss = NC_GALAXY_SD_OBS_REDSHIFT_GAUSS (test->gsdor);
  const gdouble sigma0                   = 0.05;
  const gdouble zp_max                   = 3.0;
  const gdouble rel_error                = 1.0e-5;
  const guint nsamples                   = 100;
  ...
  {
    const gdouble zp = zp_min + (zp_max - zp_min) * i / (gdouble) (nsamples - 1);
    gdouble p1, p2;
    ...
    /* Check that cached value matches */
    ncm_assert_cmpdouble_e (p1, ==, p2, 1.0e-15, 0.0);
  }

  /* Test that changing sigma0 invalidates cache */
  {
    const gdouble zp         = 1.0;
...
static void
test_nc_galaxy_sd_obs_redshift_gauss_eval_pz_given_zp (TestNcGalaxySDObsRedshift *test, gconstpointer pdata)
{
  NcGalaxySDObsRedshiftGauss *gsdorgauss = NC_GALAXY_SD_OBS_REDSHIFT_GAUSS (test->gsdor);
  const gdouble sigma0                   = 0.05;
  const gdouble rel_error                = 1.0e-5;
  ...
  {
    const guint n_integ_points = 200;
    gdouble integral           = 0.0;
    gdouble dz                 = (z_max - z_min) / (n_integ_points - 1);
    ...
    for (guint i = 0; i < n_integ_points; i++)
    {
      ...
      if ((i == 0) || (i == n_integ_points - 1))
        integral += 0.5 * p * dz;
      else
        integral += p * dz;
    }

    /* Check that integral is close to 1 (normalized distribution) */
    ncm_assert_cmpdouble_e (integral, ==, 1.0, 1.0e-2, 0.0);
  }
  ...
  /* Test that changing bin edges invalidates cache */
  {
    const gdouble z          = 0.5;
