...
test_nc_hicosmo_de_omega_x2omega_k (TestNcHICosmoDE *test, gconstpointer pdata)
{
  ...
  {
    const gdouble Omega_k  = nc_hicosmo_Omega_k (test->cosmo);
    const gdouble pOmega_k = ncm_model_param_get (NCM_MODEL (test->cosmo), NC_HICOSMO_DE_OMEGA_X);
    ncm_assert_cmpdouble_e (Omega_k, ==, pOmega_k, 1.0e-7);
  }
  ...
  {
    const gdouble Omega_k  = nc_hicosmo_Omega_k (test->cosmo);
    ncm_assert_cmpdouble_e (Omega_k, ==, 0.0, 1.0e-7);
  }
}
