    ...
    query_data = Table(
        {
            ...
            "z_query": [np.random.uniform(0.0, 1.0) for _ in range(1000)],
            "ID_query": range(1000),
        }
    )
    ...
    match_data = Table(
        {
            ...
            "z_match": [np.random.uniform(0.0, 1.0) for _ in range(1000)],
            "ID_match": range(1000),
        }
    )
...
def test_match_2d(setup_catalogs):
    ...
    for row in mt:
        ...
        for i, (matched_id, distance) in enumerate(
            ...
            assert_allclose(angular_distance, distance, atol=1.0e-16)
...
def test_match_3d(cosmo, setup_catalogs):
    ...
    mt = matching.match_3d(
        cosmo, matching_distance=np.pi / 2.0, n_nearest_neighbours=10, verbose=False
    )
    assert mt is not None
    ...
    for row in mt:
        ...
        for i, (matched_id, distance) in enumerate(
            ...
            assert_allclose(euclidean_distance, distance, atol=1.0e-16)
