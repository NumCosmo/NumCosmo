   ...
   */
  {
    ...
    if (nmassnu > 0)
    {
      if (ncm_model_vparam_len (model, NC_HICOSMO_GCG_MASSNU_T) == 0)
        ncm_model_set_vparam_len (model, NC_HICOSMO_GCG_MASSNU_T, nmassnu);

      if (ncm_model_vparam_len (model, NC_HICOSMO_GCG_MASSNU_MU) == 0)
        ncm_model_set_vparam_len (model, NC_HICOSMO_GCG_MASSNU_MU, nmassnu);

      if (ncm_model_vparam_len (model, NC_HICOSMO_GCG_MASSNU_G) == 0)
        ncm_model_set_vparam_len (model, NC_HICOSMO_GCG_MASSNU_G, nmassnu);
    }
  }
  /* Chain up : start */
...
#define MACRO_H0 (ncm_model_orig_param_get (VECTOR, NC_HICOSMO_GCG_H0))
#define OMEGA_C  (ncm_model_orig_param_get (VECTOR, NC_HICOSMO_GCG_OMEGA_C))
#define OMEGA_X  (ncm_model_orig_param_get (VECTOR, NC_HICOSMO_GCG_OMEGA_X))
#define T_GAMMA0 (ncm_model_orig_param_get (VECTOR, NC_HICOSMO_GCG_T_GAMMA0))
#define HE_YP    (ncm_model_orig_param_get (VECTOR, NC_HICOSMO_GCG_HE_YP))
#define ENNU     (ncm_model_orig_param_get (VECTOR, NC_HICOSMO_GCG_ENNU))
#define OMEGA_R  (_nc_hicosmo_gcg_Omega_gnu0 (NC_HICOSMO (cosmo)))
#define OMEGA_B  (ncm_model_orig_param_get (VECTOR, NC_HICOSMO_GCG_OMEGA_B))
#define GAMMA    (ncm_model_orig_param_get (VECTOR, NC_HICOSMO_GCG_GAMMA))
...
nc_hicosmo_gcg_omega_x2omega_k (NcHICosmoGCG *cosmo_de, GError **error)
{
  g_return_if_fail (error == NULL || *error == NULL);
  {
    ...
    NCM_UTIL_ON_ERROR_RETURN (error, ncm_reparam_free (NCM_REPARAM (gcg_reparam_ok)), );
...
nc_hicosmo_gcg_cmb_params (NcHICosmoGCG *cosmo_de, GError **error)
{
  g_return_if_fail (error == NULL || *error == NULL);
  {
    ...
    NCM_UTIL_ON_ERROR_RETURN (error, ncm_reparam_free (NCM_REPARAM (gcg_reparam_cmb)), );
