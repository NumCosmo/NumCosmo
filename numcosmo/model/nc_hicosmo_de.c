...
static gdouble _nc_hicosmo_de_neutrino_integrand (gpointer userdata, const gdouble v, const gdouble w)
{
	...
	const gdouble u = v + nudata->xi;

	return pow (u, nudata->m) * pow (u * u - nudata->xi * nudata->xi, nudata->n / 2.) * (1.0 / (w + exp (nudata->xi - nudata->yi)) + 1.0 / (w + exp (nudata->xi + nudata->yi)));
};

static gdouble _nc_hicosmo_de_neutrino_integrals (NcmIntegral1dPtr* int1d_ptr, const guint m, const guint n, const gdouble xi, const gdouble yi)
{
	NcmIntegral1d* int1d = NCM_INTEGRAL1D (int1d_ptr);

	neutrino_int* nudata = { m, n, xi, yi };
	ncm_integral1d_ptr_set_userdata (NuInt, nudata);

	gdouble Imn, err;
	Imn = ncm_integral1d_eval_gauss_laguerre (int1d, gdouble * err);
...
static gdouble _nc_hicosmo_de_neutrino_Omega_mnu (gdouble z, gpointer params)
{
	NcHICosmoDENeutrinoparams* NuParams = (NcHICosmoDENeutrinoparams*)params;
	NcmModel* model = NCM_MODEL (NuParams->cosmo_de);
	gdouble rho_mnu, rho_rnu = 0.0;
	gdouble xi, yi;
	gdouble mass_eV, Tnu, mu, g, I21, I03;
	for (guint nu_i = 0; nu_i < ncm_model_vparam_len (model, NC_HICOSMO_DE_MASSNU_T); nu_i++)
	{
		mass_eV = ncm_model_vparam_get (model, NC_HICOSMO_DE_MASSNU_M, nu_i);
		Tnu = ncm_model_vparam_get (model, NC_HICOSMO_DE_MASSNU_T, nu_i) * (1.0 + z) * nc_hicosmo_T_gamma0 (NC_HICOSMO (cosmo_de));
        xi = mass_eV * ncm_c_eV () / (ncm_c_kb () * Tnu);

		yi = ncm_model_vparam_get (model, NC_HICOSMO_DE_MASSNU_MU, nu_i) * (1.0 + z); /* the chemical potential is given is units of the neutrino temperature today (is it ??)*/
		g = ncm_model_vparam_get (model, NC_HICOSMO_DE_MASSNU_G, nu_i);

		 I21 = _nc_hicosmo_de_neutrino_integrals (NuParams->int1d_ptr, 2, 1, xi, yi);

         rho_mnu += g * gsl_pow_4(Tnu * ncm_c_kb()) / (2 * M_PI * M_PI * gsl_pow_3(ncm_c_hbar() * ncm_c_c())) * I21;

         I03 = _nc_hicosmo_de_neutrino_integrals (NuParams->int1d_ptr, 0, 3, xi, yi);

         rho_rnu += g * gsl_pow_4(Tnu * ncm_c_kb()) / (2 * M_PI * M_PI * gsl_pow_3(ncm_c_hbar() * ncm_c_c())) * I03;
...
static gdouble _nc_hicosmo_de_neutrino_Omega_rnu (gdouble z, gpointer params)
{
...
static void _nc_hicosmo_de_neutrino_prepare (NcHICosmoDE* cosmo_de)
{
}
...
_nc_hicosmo_Omega_mnu0 (NcHICosmo* cosmo, const guint nu_i, const gdouble z)
{
	...
	NcmModel* model = NCM_MODEL (cosmo);
	/*  */
	NcmIntegral1d* NuInt = NCM_INTEGRAL1D (cosmo_de->NuIntPtr);

	const gdouble m = ncm_model_vparam_get (model, NC_HICOSMO_DE_MASSNU_M, nu_i);
	const gdouble T = ncm_model_vparam_get (model, NC_HICOSMO_DE_MASSNU_T, nu_i) * (1.0 + z) * nc_hicosmo_T_gamma0 (cosmo);
	const gdouble mu = ncm_model_vparam_get (model, NC_HICOSMO_DE_MASSNU_MU, nu_i);
	const gdouble g = ncm_model_vparam_get (model, NC_HICOSMO_DE_MASSNU_G, nu_i);
	const gdouble xi = m / T;
	const gdouble yi = mu / T;

	gdouble intmn, rho_mnu, pressure;

	NcHICosmoDEneutrino_integrand_data* nudata = { 0, 0, x_i, y_i };
	ncm_integral1d_ptr_set_userdata (NuInt, nudata);
	...
	nudata->m = 2;
	nudata->n = 1;
	intmn = ncm_integral1d_eval_gauss_laguerre (NuInt, gdouble * err);
	rho_mnu = g * gsl_pow_4 (T) / (2.0 * M_PI * M_PI) * intmn;

	/* pressure */
	nudata->m = 0;
	nudata->n = 3;
	intmn = ncm_integral1d_eval_gauss_laguerre (NuInt, gdouble * err);
	pressure = g * gsl_pow_4 (T) / (6.0 * M_PI * M_PI) * intmn;
...
_nc_hicosmo_de_constructed (GObject* object)
{
	...
	G_OBJECT_CLASS (nc_hicosmo_de_parent_class)->constructed (object);
	{
		...
		const guint xi_len = ncm_model_vparam_len (model, NC_HICOSMO_DE_MASSNU_MU);
		...
		if (m_len != 0)
		{
			...
			cosmo_de->NuIntPtr = ncm_integral1d_ptr_new (&_nc_hicosmo_de_neutrino_integrand, NULL);
		}
	}
}
