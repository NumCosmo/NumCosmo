...
{
  const double H0    = 1.0 / nc_hicosmo_RH_Mpc (cosmo);
  const double T_cmb = nc_hicosmo_T_gamma0 (cosmo);
    ...
    if (N_ncdm != 0)
    {
      ...
      for (n = 0; n < N_ncdm; n++)
      {
        if (pba->m_ncdm_in_eV[n] != 0.0)
        {
          ...
          pba->M_ncdm[n] = pba->m_ncdm_in_eV[n] / _k_B_ * _eV_ / pba->T_ncdm[n] / T_cmb;
        ...
        else
        {
          ...
          /*printf("M_ncdm:%g\n",pba->M_ncdm[n]); */
          pba->m_ncdm_in_eV[n] = _k_B_ / _eV_ * pba->T_ncdm[n] * pba->M_ncdm[n] * T_cmb;
        }
...
nc_cbe_get_matter_ps (NcCBE *cbe)
{
  ...
  if (z_size < 2)
    g_error ("nc_cbe_get_matter_ps: only one redshift specified, use nc_cbe_set_max_matter_pk_z to set a the maximum redshift.");
...
nc_cbe_get_all_Cls (NcCBE *cbe, NcmVector *PHIPHI_Cls, NcmVector *TT_Cls, NcmVector *EE_Cls, NcmVector *BB_Cls, NcmVector *TE_Cls)
{
  ...
  {
    const gdouble T_gamma0 = nc_hicosmo_T_gamma0 (cbe->priv->pba.cosmo);
