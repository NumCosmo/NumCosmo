...
int
spectra_bandpower (struct spectra *psp,
                   int            l1,
                   int            l2,
                   double         *TT_II,
                   double         *TT_RI,
  ...
  if (psp->ln_tau_size == 1)
  {
    ...
    for (index_k = 0; index_k < psp->ln_k_size; index_k++)
      if (psp->ic_size[index_md] == 1)
      {
        ...
        if (pba->has_ncdm)
          output_cb_tot[index_k] = psp->ln_pk_cb[index_k];
      }
      else
      {
      ...
      for (index_ic1 = 0; index_ic1 < psp->ic_size[index_md]; index_ic1++)
      {
        for (index_ic2 = index_ic1; index_ic2 < psp->ic_size[index_md]; index_ic2++)
        {
          ...
          if (index_ic1 == index_ic2)
          {
            ...
            if (pba->has_ncdm)
              output_cb_tot[index_k] += exp (output_cb_ic[index_k * psp->ic_ic_size[index_md] + index_ic1_ic2]);
          }
          else
          {
            ...
            else
            {
              ...
              if (pba->has_ncdm)
                output_cb_ic[index_k * psp->ic_ic_size[index_md] + index_ic1_ic2] = 0.;
            }
          }
        }
      }
  ...
  if (mode == linear)
  {
    if (psp->ic_size[index_md] == 1)
    {
      for (index_k = 0; index_k < psp->ln_k_size; index_k++)
      {
        ...
        if (pba->has_ncdm)
          output_cb_tot[index_k] = exp (output_cb_tot[index_k]);
      }
    }
    else
    {
  ...
  else
  {
    if (psp->ic_size[index_md] > 1)
    {
      for (index_k = 0; index_k < psp->ln_k_size; index_k++)
      {
        ...
        if (pba->has_ncdm)
          output_cb_tot[index_k] = log (output_cb_tot[index_k]);
      }
    }
  }
  ...
  else
  {
    ...
    else
    {
      ...
      for (index_ic1 = 0; index_ic1 < psp->ic_size[index_md]; index_ic1++)
      {
        ...
        if (pba->has_ncdm)
          pk_cb_ic[index_ic1_ic2] = exp (pk_cb_ic[index_ic1_ic2]);
      }
        ...
        if (psp->is_non_zero[index_md][index_ic1_ic2] == _TRUE_)
        {
          if (index_ic1 == index_ic2)
          {
            *pk_tot += pk_ic[index_ic1_ic2];
            ...
            if (pba->has_ncdm)
              *pk_cb_tot += pk_cb_ic[index_ic1_ic2];
          }
          else
          {
            *pk_tot += 2. * pk_ic[index_ic1_ic2];
            ...
            if (pba->has_ncdm)
              *pk_cb_tot += 2. * pk_cb_ic[index_ic1_ic2];
          }
        }
        else
        {
                   ...
                   )
{
  ...
  if (psp->ln_tau_size == 1)
  {
    ...
    for (index_k = 0; index_k < psp->ln_k_size; index_k++)
    {
      ...
      if (pba->has_ncdm)
        output_cb_tot[index_k] = psp->ln_pk_cb_nl[index_k];
    }
  }
  /** - --> (b) if several values of tau have been stored, use interpolation routine to get spectra at correct redshift */
  ...
  if (ppt->has_cls == _TRUE_)
  {
    ...
    if (ppt->has_scalars == _TRUE_)
    {
      ...
      if (psp->has_tt == _TRUE_)
        psp->l_max_ct[ppt->index_md_scalars][psp->index_ct_tt] = ppt->l_scalar_max;

      if (psp->has_ee == _TRUE_)
        psp->l_max_ct[ppt->index_md_scalars][psp->index_ct_ee] = ppt->l_scalar_max;

      if (psp->has_te == _TRUE_)
        psp->l_max_ct[ppt->index_md_scalars][psp->index_ct_te] = ppt->l_scalar_max;

      if (psp->has_pp == _TRUE_)
        psp->l_max_ct[ppt->index_md_scalars][psp->index_ct_pp] = ppt->l_scalar_max;

      if (psp->has_tp == _TRUE_)
        psp->l_max_ct[ppt->index_md_scalars][psp->index_ct_tp] = ppt->l_scalar_max;

      if (psp->has_ep == _TRUE_)
        psp->l_max_ct[ppt->index_md_scalars][psp->index_ct_ep] = ppt->l_scalar_max;
    ...
    if (ppt->has_tensors == _TRUE_)
    {
      ...
      if (psp->has_tt == _TRUE_)
        psp->l_max_ct[ppt->index_md_tensors][psp->index_ct_tt] = ppt->l_tensor_max;

      if (psp->has_ee == _TRUE_)
        psp->l_max_ct[ppt->index_md_tensors][psp->index_ct_ee] = ppt->l_tensor_max;

      if (psp->has_te == _TRUE_)
        psp->l_max_ct[ppt->index_md_tensors][psp->index_ct_te] = ppt->l_tensor_max;

      if (psp->has_bb == _TRUE_)
        psp->l_max_ct[ppt->index_md_tensors][psp->index_ct_bb] = ppt->l_tensor_max;
    }
           ...
           )
{
  ...
  for (index_tau = 0; index_tau < psp->ln_tau_size; index_tau++)
  {
    for (index_k = 0; index_k < psp->ln_k_size; index_k++)
    {
      ...
      if (pba->has_ncdm)
        ln_pk_cb_tot = log (pk_cb_tot);

      psp->ln_pk_l[index_tau * psp->ln_k_size + index_k] = ln_pk_tot;

      if (pba->has_ncdm)
        psp->ln_pk_cb_l[index_tau * psp->ln_k_size + index_k] = ln_pk_cb_tot;
  ...
  class_call (spectra_sigma (pba, ppm, psp, 8. / h, 0., &(psp->sigma8)),
              psp->error_message,
              psp->error_message);
  ...
  if (psp->spectra_verbose > 0)
    fprintf (stdout, " -> sigma8=%g (computed till k = %g h/Mpc)\n",
             psp->sigma8,
             exp (psp->ln_k[psp->ln_k_size - 1]) / h);

  if (pba->has_ncdm)
  {
    class_call (spectra_sigma_cb (pba, ppm, psp, 8. / h, 0., &(psp->sigma8_cb)),
                psp->error_message,
                psp->error_message);
...
int
spectra_output_tk_titles (struct background *pba,
                          struct perturbs   *ppt,
...
int
spectra_firstline_and_ic_suffix (struct perturbs *ppt,
                                 int             index_ic,
                                 ...
                                 )
{
  ...
  for (index_k = 0; index_k < kvec_size; index_k++)
  {
    ...
    for (index_z = 0; index_z < zvec_size; index_z++)
    {
      ...
      if (pba->has_ncdm)
        pk_cb_tot_out[index_z * kvec_size + index_k] = 0.;
    }
