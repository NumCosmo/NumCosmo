                    ...
                    )
{
  ...
  if (z >= pth->z_table[pth->tt_size - 1])
  {
    ...
    /* Calculate Tb */
    pvecthermo[pth->index_th_Tb] = T_cmb * (1. + z);

    /* Calculate cb2 (cb2 = (k_B/mu) Tb (1-1/3 dlnTb/dlna) = (k_B/mu) Tb (1+1/3 (1+z) dlnTb/dz)) */
    /* note that m_H / mu = 1 + (m_H/m_He-1) Y_p + x_e (1-Y_p) */
    pvecthermo[pth->index_th_cb2] = _k_B_ / (_c_ * _c_ * _m_H_) * (1. + (1. / _not4_ - 1.) * pth->YHe + x0 * (1. - pth->YHe)) * T_cmb * (1. + z) * 4. / 3.;
                               ...
                               )
{
  ...
  z_bbn = 0.1 / (8.6173e-11 * T_cmb) - 1.0;
                                   ...
                                   )
{
  ...
  for (i = preio->rt_size - 1; i > 0; i--)
  {
    ...
    dTdz = 2. / (1 + z) * preio->reionization_table[i * preio->re_size + preio->index_re_Tb]
           ...
           (T_cmb * (1. + z) - preio->reionization_table[i * preio->re_size + preio->index_re_Tb]) / pvecback[pba->index_bg_H];
                                        ...
                                        )
{
  ...
  preco->rt_size = Nz;
  preco->H0      = H0 * _c_ / _Mpc_over_m_;
  /* preco->H0 in inverse seconds (while pba->H0 is [H0/c] in inverse Mpcs) */
  preco->YHe  = pth->YHe;
  preco->Nnow = 3. * preco->H0 * preco->H0 * Omega0_b * (1. - preco->YHe) / (8. * _PI_ * _G_ * _m_H_);
  ...
  for (i = 0; i < Nz; i++)
  {
    ...
    *(preco->recombination_table + (Nz - i - 1) * preco->re_size + preco->index_re_cb2)
      = _k_B_ / (_c_ * _c_ * _m_H_) * (1. + (1. / _not4_ - 1.) * pth->YHe + xe * (1. - pth->YHe)) * Tm * (1. - rec_dTmdlna (xe, Tm, T_cmb * (1. + z), Hz, param.fHe, param.nH0 * pow ((1 + z), 3) * 1e-6, energy_injection_rate (&param, z)) / Tm / 3.);
...
int
thermodynamics_output_titles (struct background *pba,
                              struct thermo     *pth,
...
int
thermodynamics_output_data (struct background *pba,
                            struct thermo     *pth,
