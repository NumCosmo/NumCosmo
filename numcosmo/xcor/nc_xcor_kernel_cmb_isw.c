...
_nc_xcor_kernel_cmb_isw_prepare (NcXcorKernel *xclk, NcHICosmo *cosmo)
{
  ...
  ncm_powspec_prepare_if_needed (self->ps, NCM_MODEL (cosmo));
  {
    ...
    // João: TODO: Think about these functions
    // each method will have its own const factor, but the function will not change for each method, only the const factor
    // same with z range
