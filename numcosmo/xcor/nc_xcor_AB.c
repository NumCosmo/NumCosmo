...
{
	if (b < a)
		g_error ("b must be greater or equal to a");
	...
	// 	{
	// 		printf ("%g ", ncm_matrix_get (mixing, i, j));
	// 	}
	// 	printf ("\n");
	// }
	//
	...
	// NcmMatrix* cl_th = ncm_matrix_new (ell_th_cut_off + 1, 3);
...
// void nc_xcor_AB_prepare (NcXcorAB* xcab, NcHICosmo* cosmo)
// {
// 	// nc_distance_prepare_if_needed (xcab->dist, cosmo);
// 	// nc_distance_prepare_if_needed (xcab->dist, cosmo);
// 	// nc_transfer_func_prepare (xcab->tf, cosmo);
// 	// nc_growth_func_prepare (xcab->gf, cosmo);
// 	// nc_matter_power_spectrum_prepare (xcab->ps, cosmo);
//
// 	// NcWindow* wp = nc_window_tophat_new ();
// 	// NcMatterVar* var = nc_matter_var_new (NC_MATTER_VAR_FFT, wp, xcab->tf);
...
// 	// xcab->normPS = sigma82 / sigma82_no_norm;
// }


static void nc_xcor_AB_init (NcXcorAB* xcab)
{
	xcab->a = 99;
	xcab->b = 99;
...
static void _nc_xcor_AB_dispose (GObject* object)
{
	...
	/* Chain up : end */
	G_OBJECT_CLASS (nc_xcor_AB_parent_class)
	->dispose (object);
}

static void _nc_xcor_AB_finalize (GObject* object)
{
	/* Chain up : end */
	G_OBJECT_CLASS (nc_xcor_AB_parent_class)
	->finalize (object);
...
_nc_xcor_AB_set_property (GObject* object, guint prop_id, const GValue* value, GParamSpec* pspec)
{
	...
	case PROP_ELL_LIK_MAX:
		...
		break;
	// case PROP_NELL_LIK:
	// 	xcab->nell_lik = g_value_get_uint (value);
	// 	break;
	case PROP_MIXING:
...
_nc_xcor_AB_get_property (GObject* object, guint prop_id, GValue* value, GParamSpec* pspec)
{
	...
	case PROP_ELL_LIK_MAX:
		...
		break;
	// case PROP_NELL_LIK:
	// 	g_value_set_uint (value, xcab->nell_lik);
	// 	break;
	case PROP_MIXING:
