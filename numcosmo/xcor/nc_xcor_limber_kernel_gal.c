...
{
	/* Chain up : start */
	G_OBJECT_CLASS (nc_xcor_limber_kernel_gal_parent_class)
	->constructed (object);
...
_nc_xcor_limber_kernel_gal_g_func_integrand (gdouble zz, gpointer params)
{
	...
	gdouble a = 1. - ts->chiz / nc_distance_comoving (ts->dist, ts->cosmo, zz);
	gdouble dn_dz_z = ncm_spline_eval (ts->dn_dz, zz);
	return a * dn_dz_z;
}
...
static gdouble _nc_xcor_limber_kernel_gal_g_func (gdouble z, gpointer params)
{
...
//  *
// */
// void nc_xcor_limber_kernel_gal_set_dndz (NcXcorLimberKernelGal* xclkg, GArray* z, GArray* dn_dz_array)
// {
...
static gdouble _nc_xcor_limber_kernel_gal_bias (NcXcorLimberKernelGal* xclkg, gdouble z)
{
	switch (xclkg->nknots)
	{
	case 1:
...
static gdouble _nc_xcor_limber_kernel_gal_eval (NcXcorLimberKernel* xclk, NcHICosmo* cosmo, gdouble z, gint l)//, gdouble geo_z[])
{
	NcXcorLimberKernelGal* xclkg = NC_XCOR_LIMBER_KERNEL_GAL (xclk);
	...
	gdouble dn_dz_z = ncm_spline_eval (xclkg->dn_dz, z);
	gdouble bias_z = _nc_xcor_limber_kernel_gal_bias (xclkg, z);
