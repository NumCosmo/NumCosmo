 ...
 *
*/
NcXcorLimberKernelLensing* nc_xcor_limber_kernel_lensing_new (NcDistance* dist, NcRecomb* recomb, NcmVector* Nl) //, gdouble zl, gdouble zu)
{
	return g_object_new (NC_TYPE_XCOR_LIMBER_KERNEL_LENSING, "dist", dist, "recomb", recomb, "Nl", Nl, NULL);
}
...
static void _nc_xcor_limber_kernel_lensing_add_noise (NcXcorLimberKernel* xclk, NcmVector* vp1, NcmVector* vp2, guint lmin)
{
	...
	if (xclkl->Nl == NULL) g_error ("nc_xcor_limber_kernel_lensing_noise_spec : noise spectrum empty");

	if (lmin + ncm_vector_len(vp1) > xclkl->Nlmax) g_error ("nc_xcor_limber_kernel_lensing_noise_spec : too high multipole");
...
guint _nc_xcor_limber_kernel_lensing_obs_len (NcXcorLimberKernel* xclk)
{
	NCM_UNUSED (xclk);
	return 1;
}
guint _nc_xcor_limber_kernel_lensing_obs_params_len (NcXcorLimberKernel* xclk)
{
	NCM_UNUSED (xclk);
	return 0;
}
...
static void _nc_xcor_limber_kernel_lensing_dispose (GObject* object)
{
	NcXcorLimberKernelLensing* xclkl = NC_XCOR_LIMBER_KERNEL_LENSING (object);
	...
	/* Chain up : end */
	G_OBJECT_CLASS (nc_xcor_limber_kernel_lensing_parent_class)
	->dispose (object);
}
