...
static void
nc_recomb_seager_dispose (GObject* object)
{
	NcRecombSeager* recomb_seager = NC_RECOMB_SEAGER (object);
...
nc_recomb_seager_class_init (NcRecombSeagerClass* klass)
{
	...
	object_class->dispose = nc_recomb_seager_dispose;
...
nc_recomb_seager_prepare (NcRecomb* recomb, NcHICosmo* cosmo)
{
...

	{
		...
		GArray* XHII_a = g_array_new (FALSE, FALSE, sizeof (gdouble));
		GArray* XHeII_a = g_array_new (FALSE, FALSE, sizeof (gdouble));

		{
			const guint lenXe = ncm_spline_get_len (recomb->Xe_s) - 1;
			guint i;
 ...
 */
gdouble
nc_recomb_seager_XHII (NcRecombSeager* recomb_seager, NcHICosmo* cosmo, const gdouble lambda)
{
	NCM_UNUSED (cosmo);
	return ncm_spline_eval (recomb_seager->XHII_s, lambda);
}

/**
 * nc_recomb_seager_XHeII:
 * @recomb_seager: a #NcRecombSeager
 * @cosmo: a #NcHICosmo.
 * @lambda: $\lambda$.
 *
 ...
 */
gdouble
nc_recomb_seager_XHeII (NcRecombSeager* recomb_seager, NcHICosmo* cosmo, const gdouble lambda)
{
	NCM_UNUSED (cosmo);
	return ncm_spline_eval (recomb_seager->XHeII_s, lambda);
}
