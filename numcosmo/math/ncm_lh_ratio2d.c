...
ncm_lh_ratio2d_constructed (GObject *object)
{
  ...
  G_OBJECT_CLASS (ncm_lh_ratio2d_parent_class)->constructed (object);
  {
    ...
    NcmFitState *fstate = ncm_fit_peek_state (lhr2d->fit);
    NcmLikelihood *lh   = ncm_fit_peek_likelihood (lhr2d->fit);
    ...
    for (i = 0; i < 2; i++)
    {
      ...
      if (ncm_mset_peek (fit_mset, lhr2d->pi[i].mid) == NULL)
        g_error ("ncm_lh_ratio2d_constructed: cannot use parameter[%d:%u], model not set.",
                 lhr2d->pi[i].mid, lhr2d->pi[i].pid);

      if (ncm_mset_param_get_ftype (fit_mset, lhr2d->pi[0].mid, lhr2d->pi[0].pid) != NCM_PARAM_TYPE_FREE)
      ...
      lhr2d->lb[i] = ncm_mset_param_get_lower_bound (fit_mset, lhr2d->pi[i].mid, lhr2d->pi[i].pid);
      lhr2d->ub[i] = ncm_mset_param_get_upper_bound (fit_mset, lhr2d->pi[i].mid, lhr2d->pi[i].pid);
      lhr2d->bf[i] = ncm_mset_param_get (fit_mset, lhr2d->pi[i].mid, lhr2d->pi[i].pid);
    }

    lhr2d->constrained = ncm_fit_copy_new (lhr2d->fit, lh, mset,
                                           ncm_fit_get_grad_type (lhr2d->fit));
    ncm_mset_free (mset);
...
{
  NcmMSet *fit_mset         = ncm_fit_peek_mset (lhr2d->fit);
  NcmMSet *constrained_mset = ncm_fit_peek_mset (lhr2d->constrained);
  NcmMSetPIndex pi[2]       = {*pi1, *pi2};
  gint i;

  for (i = 0; i < 2; i++)
  {
    if (ncm_mset_param_get_ftype (fit_mset, pi[i].mid, pi[i].pid) != NCM_PARAM_TYPE_FREE)
    ...
    lhr2d->lb[i] = ncm_mset_param_get_lower_bound (fit_mset, pi[i].mid, pi[i].pid);
    lhr2d->ub[i] = ncm_mset_param_get_upper_bound (fit_mset, pi[i].mid, pi[i].pid);
    lhr2d->bf[i] = ncm_mset_param_get (fit_mset, pi[i].mid, pi[i].pid);
  }
...
ncm_lh_ratio2d_f (gdouble x, gpointer ptr)
{
  ...
  NcmMSet *constrained_mset       = ncm_fit_peek_mset (lhr2d->constrained);
  NcmFitState *fit_fstate         = ncm_fit_peek_state (lhr2d->fit);
  NcmFitState *constrained_fstate = ncm_fit_peek_state (lhr2d->constrained);
  gboolean skip                   = FALSE;
  gdouble p[2];
  gdouble res;
