...
{
  NcmBinSplit **bs_ptr = _ncm_sf_mp_0F1_get_bs ();
...
*/
void
ncm_sf_mp_0F1 (mpq_t b, mpq_t x, mpfr_ptr res, mp_rnd_t rnd)
{
	_taylor_0F1 (b, x, res, rnd);
}

/**
 * ncm_sf_mp_0F1_d: (skip)
 * @b: FIXME
 * @x: FIXME
 * @res: FIXME
 * @rnd: FIXME
...
*/
void
ncm_sf_mp_0F1_d (gdouble b, gdouble x, mpfr_ptr res, mp_rnd_t rnd)
{
  mpq_t xq, bq;
  mpq_init (xq);
	mpq_init (bq);
...
ncm_sf_0F1 (gdouble b, gdouble x)
{
  ...
  gdouble res_d;
  ncm_sf_mp_0F1_d (b, x, res, GMP_RNDN);
