...
ncm_spectral_init (NcmSpectral *spectral)
{
  ...
  printf ("[NCM_SPECTRAL_INIT] Created new NcmSpectral: %p\n", (void *) spectral);
  printf ("[NCM_SPECTRAL_INIT]   max_order=%u, coeffs=%p\n", spectral->max_order, (void *) spectral->coeffs);
}
...
ncm_spectral_set_max_order (NcmSpectral *spectral, guint max_order)
{
  ...
  printf ("[NCM_SPECTRAL_SET_MAX_ORDER] Setting max_order: old=%u, new=%u\n", spectral->max_order, max_order);

  if (spectral->max_order != max_order)
  {
    spectral->max_order = max_order;

    printf ("[NCM_SPECTRAL_SET_MAX_ORDER] max_order changed, clearing cached data\n");

    /* Clear cached plans and arrays as they depend on max_order */
    if (spectral->fftw_plans != NULL)
    {
      printf ("[NCM_SPECTRAL_SET_MAX_ORDER] Clearing fftw_plans (len=%u)\n", spectral->fftw_plans->len);
      g_ptr_array_unref (spectral->fftw_plans);
      spectral->fftw_plans = NULL;
    }
    ...
    if (spectral->cos_arrays != NULL)
    {
      printf ("[NCM_SPECTRAL_SET_MAX_ORDER] Clearing cos_arrays (len=%u)\n", spectral->cos_arrays->len);
      g_ptr_array_unref (spectral->cos_arrays);
      spectral->cos_arrays = NULL;
    }
    ...
    if (spectral->f_vals != NULL)
    {
      printf ("[NCM_SPECTRAL_SET_MAX_ORDER] Freeing f_vals\n");
      fftw_free (spectral->f_vals);
      spectral->f_vals = NULL;
    }
    ...
    if (spectral->coeffs_work != NULL)
    {
      printf ("[NCM_SPECTRAL_SET_MAX_ORDER] Freeing coeffs_work\n");
      fftw_free (spectral->coeffs_work);
      spectral->coeffs_work = NULL;
    }
    ...
    {
      printf ("[NCM_SPECTRAL_SET_MAX_ORDER] Clearing spectral->coeffs (current len=%u)\n", spectral->coeffs->len);
      g_array_set_size (spectral->coeffs, 0);
    }

    printf ("[NCM_SPECTRAL_SET_MAX_ORDER] Cleanup complete\n");
  }
  else
  {
    printf ("[NCM_SPECTRAL_SET_MAX_ORDER] max_order unchanged, no action needed\n");
  }
}
...
ncm_spectral_get_max_order (NcmSpectral *spectral)
{
  ...
  printf ("[NCM_SPECTRAL_GET_MAX_ORDER] Returning max_order=%u for spectral=%p\n", max_order, (void *) spectral);
...
_ncm_spectral_check_convergence (GArray *coeffs_2N, GArray *coeffs_N, gdouble tol)
{
  ...
  printf ("[NCM_SPECTRAL_CONVERGENCE] Checking convergence: N_len=%u, 2N_len=%u, tol=%.15e\n",
  ...
  printf ("[NCM_SPECTRAL_CONVERGENCE] norm2_diff=%.15e, norm2_2N=%.15e, threshold=%.15e, converged=%d\n",
                                                ...
                                                gdouble tol, GArray **coeffs, gpointer user_data)
{
  ...
  printf ("[NCM_SPECTRAL_ADAPTIVE] Starting adaptive computation:\n");
  printf ("[NCM_SPECTRAL_ADAPTIVE]   max_order=%u, k_min=%u, tol=%.15e\n", spectral->max_order, k_min, tol);
  printf ("[NCM_SPECTRAL_ADAPTIVE]   interval=[%.15e, %.15e]\n", a, b);
  printf ("[NCM_SPECTRAL_ADAPTIVE]   coeffs=%p, *coeffs=%p\n", (void *) coeffs, (void *) *coeffs);
  printf ("[NCM_SPECTRAL_ADAPTIVE]   spectral->coeffs=%p, len=%u\n", (void *) spectral->coeffs, spectral->coeffs->len);
  ...
  printf ("[NCM_SPECTRAL_ADAPTIVE] Clearing spectral->coeffs (current len=%u)\n", spectral->coeffs->len);
  g_array_set_size (spectral->coeffs, 0);

  /* Initial evaluation at k_min */
  printf ("[NCM_SPECTRAL_ADAPTIVE] Initial evaluation at k=%u (N=%u)\n", k, (1 << k) + 1);
  ...
  printf ("[NCM_SPECTRAL_ADAPTIVE] Array pointers: c_tmp=%p, c_final=%p\n", (void *) c_tmp, (void *) c_final);
  printf ("[NCM_SPECTRAL_ADAPTIVE] c_tmp is spectral->coeffs: %d\n", c_tmp == spectral->coeffs);
  printf ("[NCM_SPECTRAL_ADAPTIVE] c_final is *coeffs: %d\n", c_final == *coeffs);
  ...
  /* Transform using N and store in coeffs_work */
  {
    ...
    printf ("[NCM_SPECTRAL_ADAPTIVE] Initial transform: k=%u, N=%u\n", k, N);

    /* Transform f_vals -> coeffs_work */
    fftw_execute (plan);

    printf ("[NCM_SPECTRAL_ADAPTIVE] Setting c_tmp size to %u (current: %u)\n", N, c_tmp->len);
    g_array_set_size (c_tmp, N);
    _ncm_spectral_normalize_coeffs (spectral->coeffs_work, c_tmp, N);

    printf ("[NCM_SPECTRAL_ADAPTIVE] After normalization: c_tmp[0]=%.15e, c_tmp[N-1]=%.15e\n",
            g_array_index (c_tmp, gdouble, 0), g_array_index (c_tmp, gdouble, N - 1));
  }

  while (k < spectral->max_order)
  {
    printf ("[NCM_SPECTRAL_ADAPTIVE] === Refinement iteration: k=%u -> k=%u ===\n", k, k + 1);
    ...
    k++;
    {
      ...
      printf ("[NCM_SPECTRAL_ADAPTIVE] Refined transform: k=%u, N=%u\n", k, N);

      /* Transform f_vals -> coeffs_work */
      fftw_execute (plan);

      printf ("[NCM_SPECTRAL_ADAPTIVE] Setting c_final size to %u (current: %u)\n", N, c_final->len);
      g_array_set_size (c_final, N);
      _ncm_spectral_normalize_coeffs (spectral->coeffs_work, c_final, N);

      printf ("[NCM_SPECTRAL_ADAPTIVE] After normalization: c_final[0]=%.15e, c_final[N-1]=%.15e\n",
              g_array_index (c_final, gdouble, 0), g_array_index (c_final, gdouble, N - 1));
    }

    printf ("[NCM_SPECTRAL_ADAPTIVE] Before convergence check: c_tmp len=%u, c_final len=%u\n",
            c_tmp->len, c_final->len);

    /* Check convergence using pre-computed e_total */
    if (_ncm_spectral_check_convergence (c_final, c_tmp, tol))
    {
      printf ("[NCM_SPECTRAL_ADAPTIVE] Converged at k=%u\n", k);
      break;
    }

    printf ("[NCM_SPECTRAL_ADAPTIVE] Not converged, continuing refinement\n");

    /* Swap c_tmp and c_final */
    {
      GArray *tmp = c_tmp;

      printf ("[NCM_SPECTRAL_ADAPTIVE] Swapping arrays: c_tmp=%p -> c_final=%p\n", (void *) c_tmp, (void *) c_final);
      c_tmp   = c_final;
      c_final = tmp;
      printf ("[NCM_SPECTRAL_ADAPTIVE] After swap: c_tmp=%p, c_final=%p\n", (void *) c_tmp, (void *) c_final);
    }
  }

  printf ("[NCM_SPECTRAL_ADAPTIVE] Refinement loop complete: final k=%u\n", k);
  printf ("[NCM_SPECTRAL_ADAPTIVE] c_final=%p, *coeffs=%p, need_copy=%d\n",
          (void *) c_final, (void *) *coeffs, c_final != *coeffs);

  if (c_final != *coeffs)
  {
    /* If final coefficients are not in *coeffs, copy them */
    printf ("[NCM_SPECTRAL_ADAPTIVE] Copying c_final (len=%u) to *coeffs\n", c_final->len);
    g_array_set_size (*coeffs, c_final->len);
    memcpy ((*coeffs)->data, c_final->data, sizeof (gdouble) * c_final->len);
  }

  printf ("[NCM_SPECTRAL_ADAPTIVE] Final result: k=%u, coeffs_len=%u\n", k, (*coeffs)->len);
  printf ("[NCM_SPECTRAL_ADAPTIVE] Final coeffs[0]=%.15e, coeffs[N-1]=%.15e\n",
