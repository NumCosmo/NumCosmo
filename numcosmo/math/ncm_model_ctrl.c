 ...
 * @title: GModel update control object
...
{
	if (model)
		return g_object_new (NCM_TYPE_GMODEL_CTRL, "model", model, NULL);
	else
		return g_object_new (NCM_TYPE_GMODEL_CTRL, NULL);
}
...
{
	NcmModelCtrl *ctrl = NCM_MODEL_CTRL (object);
	g_return_if_fail (NCM_IS_GMODEL_CTRL (object));
