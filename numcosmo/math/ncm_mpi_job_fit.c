...
typedef struct _NcmMPIJobFitPrivate
{
  NcmFit *fit;
  NcmObjArray *func_oa;
  guint fparam_len;
...
static NcmMPIDatatype _ncm_mpi_job_fit_input_datatype (NcmMPIJob *mpi_job, gint *len, gint *size);
static NcmMPIDatatype _ncm_mpi_job_fit_return_datatype (NcmMPIJob *mpi_job, gint *len, gint *size);
...
_ncm_mpi_job_fit_run (NcmMPIJob *mpi_job, gpointer input, gpointer ret)
{
  ...
  ncm_mset_fparams_get_vector_offset (mset, ret, 1);
