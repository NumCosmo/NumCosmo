...
_ncm_spline_notaknot_prepare_base (NcmSpline *s)
{
  ...
  for (i = 1; i <= nm3; i++)
  {
	...
	ncm_vector_set (sc->offdiag, i, h_ip1);
	ncm_vector_set (sc->diag, i,    2.0 * (h_ip1 + h_i));
	ncm_vector_set (sc->g, 1 + i,   3.0 * (ydiff_ip1 * g_ip1 -  ydiff_i * g_i));
  }

  {
	...
	{
	  start_i = 1;
	  ncm_vector_set (sc->c, 1, 3.0 * (ydiff_1 -  ydiff_0 * h_1 / h_0) / ((h_1 + h_0) * (2.0 * h_1 + h_0)));
	  ncm_vector_subfrom (sc->g, 1 + 1, h_1 * ncm_vector_get (sc->c, 1));
	}
	else
	{
	  ncm_vector_set (sc->offdiag, 0, h_1);
	  ncm_vector_set (sc->diag, 0,    h_1 * (2.0 * h_1 + h_0) / (h_1 - h_0));
	  ncm_vector_set (sc->g, 1 + 0,   3.0 * h_1 * (ydiff_1 -  ydiff_0 * h_1 / h_0) / (h_1 * h_1 - h_0 * h_0));
	}
  }

  {
	...
	{
	  pad_i = 1;
	  ncm_vector_set (sc->c, nm1, 3.0 * (ydiff_nm1 * h_nm2 / h_nm1 - ydiff_nm2) / ((h_nm2 + h_nm1) * (2.0 * h_nm2 + h_nm1)));
	  ncm_vector_subfrom (sc->g, 1 + nm1 - 2, h_nm2 * ncm_vector_get (sc->c, nm1));
	}
	else
	{
	  ncm_vector_set (sc->offdiag, nm2, h_nm1);
	  ncm_vector_set (sc->diag,    nm2, h_nm2 * (2.0 * h_nm2 + h_nm1) / (h_nm2 - h_nm1));
	  ncm_vector_set (sc->g,   1 + nm2, 3.0 * h_nm2 * (ydiff_nm1 * h_nm2 / h_nm1 - ydiff_nm2) / (h_nm2 * h_nm2 - h_nm1 * h_nm1));
	}
  }
...
#ifdef HAVE_LAPACKA
	{
	  printf ("# nhocC[%zu] % 20.15g\n", nm2, ncm_vector_get (sc->diag, nm2));
	  ...
	  if (info != 0)
	  {
		for (i = 0; i < s->len; i++)
		{
		  printf ("[%zu/%zu] [%zu, %zu] % 20.15g %20.15g [% 20.15g % 20.15g]\n", i, loc_sys_size, start_i, pad_i, ncm_vector_get (s->xv, i), ncm_vector_get (s->yv, i),
		          ncm_vector_get (sc->diag, i),
		          ncm_vector_get (sc->offdiag, i));
		}
...
#endif
	{
	  const gdouble c1   = ncm_vector_get (sc->c, 1);
	  const gdouble c2   = ncm_vector_get (sc->c, 2);
	  const gdouble cnm1 = ncm_vector_get (sc->c, nm1);
	  const gdouble cnm2 = ncm_vector_get (sc->c, nm2);

	  ncm_vector_set (sc->c, 0, c1 + h_0 * (c1 - c2) / h_1);
	  ncm_vector_set (sc->c, n, cnm1 + h_nm1 * (cnm1 - cnm2) / h_nm2);
	}
...
_ncm_spline_notaknot_prepare (NcmSpline *s)
{
  ...
  for (i = 0; i < n; i++)
  {
	...
	ncm_vector_set (sc->b, i, (dy / dx) - dx * (c_ip1 + 2.0 * c_i) / 3.0);
	ncm_vector_set (sc->d, i, (c_ip1 - c_i) / (3.0 * dx));
  }
