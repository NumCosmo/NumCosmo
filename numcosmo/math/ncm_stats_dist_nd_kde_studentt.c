 ...
 * @short_description: An N dimensional probability distributions using studenttian KDE
 *
 * An arbitrary N dimensional probability distribution using studenttian KDE.
 * 
 */
...
_ncm_stats_dist_nd_kde_studentt_set_property (GObject *object, guint prop_id, const GValue *value, GParamSpec *pspec)
{
  ...
  switch (prop_id)
  {
    ...
    case PROP_LOOCV:
      ...
      break;
    case PROP_FREEDOM:
      self->st_freedom = g_value_get_uint (value);
      break;
    default:
...
_ncm_stats_dist_nd_kde_studentt_get_property (GObject *object, guint prop_id, GValue *value, GParamSpec *pspec)
{
  ...
  switch (prop_id)
  {
    ...
    case PROP_LOOCV:
      ...
      break;
     case PROP_FREEDOM:
     g_value_set_uint (value, self->st_freedom);
     break;
    default:
      G_OBJECT_WARN_INVALID_PROPERTY_ID (object, prop_id, pspec);
      break;
...
gdouble _ncm_stats_dist_nd_kde_studentt_studentt (NcmStatsDistNdKDEStudenttPrivate * const self, gdouble st_value);
...
ncm_stats_dist_nd_kde_studentt_class_init (NcmStatsDistNdKDEStudenttClass *klass)
{
  ...
  g_object_class_install_property (object_class,
                                   PROP_FREEDOM,
                                   g_param_spec_uint("FREEDOM",
                                                     NULL,
                                                     "Freedom value of the function",
                                                      1, G_MAXUINT, 200,
...
_ncm_stats_dist_nd_kde_studentt_prepare (NcmStatsDistNd *dnd)
{
  ...
  self->us_lnnorm = log(gsl_sf_gamma((self->st_freedom + self->d) / 2)) - log(gsl_sf_gamma(self->st_freedom / 2)) - (self->d / 2) * (log(M_PI) + log(self->st_freedom)) -(1/2) * ncm_matrix_cholesky_lndet (self->cov_decomp);
...
return st_result =pow((1 + st_value /self->st_freedom),-((self->st_freedom +self->d)/2));
}
static void 
...
_ncm_stats_dist_nd_kde_studentt_LOOCV_err2_full_levmar_f (gdouble *x, gdouble *hx, gint m, gint n, gpointer f_data)
{
  ...
  self->href      = 1.0;
  self->href2     = self->href * self->href;
  self->us_lnnorm =log(gsl_sf_gamma((self->st_freedom + self->d) / 2)) - log(gsl_sf_gamma(self->st_freedom / 2)) - (self->d / 2) * (log(M_PI) + log(self->st_freedom)) -0.5 * ncm_matrix_cholesky_lndet (self->cov_decomp);
  self->lnnorm    = self->us_lnnorm;
...
printf("det: %lf \n norma:  %lf \n, resultado: %lf",ncm_matrix_cholesky_lndet (self->cov_decomp), self->lnnorm, log (s));
...
_ncm_stats_dist_nd_kde_studentt_kernel_sample (NcmStatsDistNd *dnd, NcmVector *y, NcmVector *mu, gdouble scale, NcmRNG *rng)
{
  ...
  chi_scale = pow((self->st_freedom/gsl_ran_chisq(rng->r, self->st_freedom)),0.5);
...
ncm_stats_dist_nd_kde_studentt_new (const guint dim, const gboolean LOOCV, const guint FREEDOM)
{
