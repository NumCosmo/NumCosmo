...
_ncm_spline_bspline_prepare (NcmSpline *s)
{
  ...
  ret = gsl_bspline_init_interp (&xv.vector, sbs->w);

  if (ret != GSL_SUCCESS)
    g_error ("_ncm_spline_bspline_prepare: gsl_bspline_init_interp failed: %s.", gsl_strerror (ret));

  ret = gsl_bspline_col_interp (&xv.vector, sbs->XB, sbs->w);

  if (ret != GSL_SUCCESS)
    g_error ("_ncm_spline_bspline_prepare: gsl_bspline_col_interp failed: %s.", gsl_strerror (ret));

  ret = gsl_linalg_LU_band_decomp (s_len, band, band, sbs->XB, sbs->piv);

  if (ret != GSL_SUCCESS)
    g_error ("_ncm_spline_bspline_prepare: banded LU decomposition failed: %s.", gsl_strerror (ret));

  ret = gsl_linalg_LU_band_solve (band, band, sbs->XB, sbs->piv, &yv.vector, sbs->c);

  if (ret != GSL_SUCCESS)
    g_error ("_ncm_spline_bspline_prepare: banded LU solve failed: %s.", gsl_strerror (ret));
}
