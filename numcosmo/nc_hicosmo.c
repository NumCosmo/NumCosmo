 ...
 */
/**
 * nc_hicosmo_curvature_K:
 * @cosmo: a #NcHICosmo
 * @sgnK: (out): signal of $K$
 *
 * Computes the curvature constant $K = - \Omega_{k0} / R_H^2$
 * in $\mathrm{Mpc}^{-2}$. If @sgn is not null the sign of $K$
...
{
  NcmPowspecFilterType type = ncm_powspec_filter_get_filter_type (psf);

  if (type != NCM_POWSPEC_FILTER_TYPE_TOPHAT)
