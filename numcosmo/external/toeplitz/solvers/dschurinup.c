...
int d_schur_innerup(fdouble *x,fdouble *mu,finteger N,fdouble *b,
...
#define d(I,J)      d__[(I) + (J) * ldsmall]
  ...
  /* space for storing theta */
  ihelp+=(k_max+2)*sizeof(fdouble);
  ...
  while(n<N-1)
  {
	...
	if (!get_anti_regular_up_schurlev(work,lwork,theta,NULL,
	      NULL,NULL,&e_up(n,0),&e_lef[n-1],&p_up(n,0),&p_lef[n-1],NULL,
	      perm,tau,&ldsmall,&ld,N,n,k))  break;

	get_alpha_beta_up_from_theta(schur_par,theta,N,n,k);

	/* calculating new q_lef */
#ifdef SCHUR_CALCULATE_WITH_D
	if (!decomp_matrix(work,&k,&d(0,0),&ldsmall,perm,tau)) break;
	if (!get_anti_regular_lef_schurlev(work,lwork,theta,
	      NULL,NULL,&e(n,0),&e_lef[n-1],&p(n,0),&p_lef[n-1],&d(0,0),
	      perm,tau,&ldsmall,&ld,N,n,k)) break;

#else
	if (!get_anti_regular_lef_schurlev(work,lwork,theta,
	      NULL,NULL,&e(n,0),&e_lef[n-1],&p(n,0),&p_lef[n-1],NULL,perm,
	      tau,&ldsmall,&ld,N,n,k)) break;
#endif
	...
	get_alpha_beta_lef_from_theta(schur_par,theta,v,N,n,k);
