...
nc_confidence_region_new_1d (NcmFit *fit, NcmModelID gmid, guint pid)
{
  ...
  cr->constrained = ncm_fit_new (fit->lh, mset, fit->type, fit->gtype);
  ncm_mset_free (mset);
...
nc_confidence_region_new (NcmFit *fit, NcmModelID gmid1, guint pid1, NcmModelID gmid2, guint pid2)
{
  ...
  cr->constrained = ncm_fit_new (fit->lh, mset, fit->type, fit->gtype);
  ncm_mset_free (mset);
  cr->covar_orto = ncm_matrix_new (2, 2);
  cr->covar_ev   = ncm_vector_new (2);
...
/*
  ...
  cr->best = fit;
  cr->constrained = ncm_fit_new_with_types (clh, mset, fit->type, fit->gtype, profile);
  cr->fit = ncm_fit_new_with_types (fit->lh, mset, fit->type, fit->gtype, cr_fit);
  cr->chi2 = gsl_cdf_chisq_Qinv (1.0 - p, nu);
...
ncm_fit_cr_root_steffenson (NcConfidenceRegion *cr, gdouble x)
{
  ...
  if (cr->constrained->gtype != NCM_FIT_GRAD_ANALYTICAL)
  {
...
ncm_fit_cr_step (NcConfidenceRegion *cr, gdouble x)
{
  ...
  } while (cr->minimize && !ncm_fit_run (cr->constrained, NC_BF_MAX_ITER, NCM_FIT_RUN_MSGS_NONE) && i--);
  cr->total_func_eval += cr->constrained->n_func_eval;
...
nc_confidence_region_df (gdouble x, gpointer p)
{
  ...
  {
	case NC_CONFIDENCE_REGION_SEARCH_1D:
	  cr->constrained->m2lnL_grad (cr->constrained, df);
	  res = df_data[0];
	  break;
	case NC_CONFIDENCE_REGION_SEARCH_ELLIPTIC_RADIUS:
	{
