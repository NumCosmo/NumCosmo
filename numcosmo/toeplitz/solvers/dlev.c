...
int d_lev(fdouble *x,fdouble *mu,finteger N,fdouble *b,
...
#define d(I,J)      d__[(I) + (J) * ldsmall]
  ...
  /* space for M */
  ihelp+=4*k_max*k_max*sizeof(fdouble);
  /* space for e_lef,p_lef,v,v_up */
  ihelp+=4*ldsmall*sizeof(fdouble);
  /* space for perm or tau */
#ifdef USE_QR_DECOMP
  ihelp+=ldM*(sizeof(fdouble)+sizeof(finteger));
#else
  ihelp+=ldM*sizeof(finteger);
#endif
  /* space for temporary variables */
#ifdef USE_QR_DECOMP
  lwork=max(12*k_max + 4*k_max*k_max,2*ldq + 9*k_max + k_max*3
	+ liwork(2*k_max));
#else
  lwork=max(12*k_max + 4*k_max*k_max,2*ldq + 9*k_max + liwork(2*k_max));
#endif
  ...
  while(n<N-1)
  {
	...
	if (!update_x(work,lwork,&q[storage_index],&q_up[storage_index],&ldq,
		      b,x,&p(0,0),&ldsmall,perm,tau,n,1))  break;
	...
	if (n+1 == N)
	{
	  ...
	  else
	  {
	    ...
	    if (!update_x(work,lwork,&q[storage_index],&q_up[storage_index],
			  &ldq,b,x,&p(0,0),&ldsmall,perm,tau,n,1))  break;
...
/*       e(0,0) = lau_coef(&h,&q[0],n+1); */
	...
	k++;
	get_M(M,&ldM,e_lef,&e_up(0,0),p_lef,&p_up(0,0),k);
	...
	k++;
      ...
      /* update solution */
      F77CALL (dgemm) ("Transpose","No transpose",&k,&k,&k,&one,&q->data[n],&ldq,
	     &e_up(0,0),&ldsmall,&zero,&d(0,0),&ldsmall);
	...
	get_anti_regular_pair(work,lwork,M,&ldM,&q_lef,q_up,&ldq,&e_up(0,0),
			      e_lef,&p_up(0,0),p_lef,n,k);

/* 	e_up(0,0) = lau_coef(&h,&q_up[0],n+k); */
/*      is now computed in get_anti_regular_pair */
...
/*      is now computed in get_anti_regular_pair */
	...
	if (n+k < N)
	{
	  ...
	  if (column_regular || ((n+k+1 == N) && (sing_tol < fabs(p(0,0)))) )
	  {
	    ...
	    if (!update_x(work,lwork,&q[storage_index],&q_up[storage_index],
			  &ldq,b,x,&p(0,0),&ldsmall,perm,tau,n+k,1))
	      break;
