...
int d_schur_inner(fdouble *x,fdouble *mu,finteger N,fdouble *b,
...
#define L(I,J)      L__[(I) + (J) * ld]
  ...
  /* space for theta */
  ihelp+=sizeof(fdouble)*(k_max+2);
  /* space for storing lower triangular matrix L containing pi's */
  ihelp+=ld*ld*sizeof(fdouble);
  /* space (finteger) for storing regular indices */
  ihelp+=ld*sizeof(finteger);
  /* space for tau or perm */
#ifdef USE_QR_DECOMP
  ...
  lwork=k_max*k_max+k_max+max(ld,liwork(k_max));
#ifdef USE_QR_DECOMP
  lwork+=k_max*3;
#endif
  lwork=max(lwork,6*k_max+k_max*k_max);
  ihelp+=lwork*sizeof(fdouble);
  p__=(fdouble *)(data=malloc(ihelp));
  if (data==NULL)
  {
  ...
  y=&b_tmp[ld];
  d__=&y[ld];
  L__=&d__[ldsmall*ldsmall];
  work=&L__[ld*ld];
#ifdef USE_QR_DECOMP
  tau=&work[lwork];
  perm=(finteger*)&tau[ldsmall];
  reg_index=&perm[ldsmall];
#else
  tau=NULL;
  perm=(finteger*)&work[lwork];
  reg_index=&perm[ldsmall];
#endif 
  ...
  /* store p in lower triangular matrix L */
  F77CALL (dcopy) (&h.N,&p(0,0),&inc1,&L(0,0),&inc1);
  ...
  {
    /* update the solution */
    update_Dy_schur(work,lwork,b_tmp,y,&e_up(0,0),&e_up(0,0),&ld,N,0,1,&ld);
  ...
  while(n<N-1)
  {
    if (column_regular) /* n column regular */
    {
      ...
      /* store p in lower triangular matrix L */
      ihelp = N-n;
      F77CALL (dcopy) (&ihelp,&p(n,storage_index),&inc1,&L(n,n),&inc1);

      if (fabs(p(n,storage_index)) > col_tol)  /* n column regular */
      {
	if (!update_Dy_schur(work,lwork,&b_tmp[n],&y[n],&e_up(n,storage_index),
		      &p(n,storage_index),&ld,
		      N,n,1,&ld))  break;
	...
	set_new_singular_value(1,p(n,storage_index));
        ...
        /* preparing arrays for the first look-ahead step */
	if (storage_index == 1)
	{
	...
	v[k]    = -e(n+k-1,k-1)/e_lef[n-1];
        ...
        get_inner_q_schurlev(NULL,NULL,NULL,&e(n+k,k),&e_lef[n-1],
                 &e(n+k-1,k-1),&p(n,k),&p_lef[n-1],&p(n,k-1),&v[k],N,n,k);

	/* store p in lower triangular matrix L */
	ihelp = N-n;
	F77CALL (dcopy) (&ihelp,&p(n,k),&inc1,&L(n,n+k),&inc1);
	...
	k++;
        ...
        update_d_schur(&d(0,0),&ldsmall,&e_up(n,0),&ld,&p(n,0),&ld,k);
#ifdef SCHUR_CALCULATE_WITH_D
        help = get_singularity(work,lwork,&d(0,0),&ldsmall,k);
#else
        help = get_singularity(work,lwork,&p(n,0),&ld,k);
        help2 = get_singularity(work,lwork,&e_up(n,0),&ld,k);
        help = min(help,help2);
#endif
        ...
        if (k==1)
	{
	  ...
	  /* update solution */
	  if (!update_Dy_schur(work,lwork,&b_tmp[n],&y[n],&e_up(n,0),&p(n,0),
			      &ld, N,n,1,&ld))  break;
      ...
      /* update solution */
      if (!update_Dy_schur(work,lwork,&b_tmp[n],&y[n],&e_up(n,0),&d(0,0),
			  &ldsmall, N,n,k,&ld))  break;
	...
	/* calculating anti-regular q_up, no calculate with d because 
	   therefore q_up is needed */
	if (!get_anti_regular_up_schurlev(work,lwork,theta,NULL,NULL,NULL,
	      &e_up(n,0),&e_lef[n-1],&p_up(n,0),&p_lef[n-1],NULL,
	      perm,tau,&ldsmall,&ld,N,n,k))  break;


	/* calculating new q_lef */
#ifdef SCHUR_CALCULATE_WITH_D
	  if (!decomp_matrix(work,&k,&d(0,0),&ldsmall,perm,tau)) break;
	  if (!get_anti_regular_lef_schurlev(work,lwork,theta,NULL,NULL,
		&e(n,0),&e_lef[n-1],&p(n,0),&p_lef[n-1],&d(0,0),perm,tau,
		&ldsmall,&ld,N,n,k)) break;
#else
	  if (!get_anti_regular_lef_schurlev(work,lwork,theta,NULL,NULL,\
		&e(n,0),&e_lef[n-1],&p(n,0),&p_lef[n-1],NULL,perm,tau,
		&ldsmall,&ld,N,n,k)) break;
#endif
	...
	get_regular_q_schurlev(NULL,NULL,NULL,&e(n+k,0),&e_lef[n+k-1],
			       &e_up(n+k,0),&p(n+k,0),&p_lef[n+k-1],
			       &p_up(n+k,0),N,n,k);

	/* store p in lower triangular matrix L */
	ihelp = N-n-k;
	F77CALL (dcopy) (&ihelp,&p(n+k,0),&inc1,&L(n+k,n+k),&inc1);
	...
	if (column_regular || ((n+k+1 == N) && (sing_tol < fabs(p(n+k,0)))) )
	{
	  ...
	  if (!update_Dy_schur(work,lwork,&b_tmp[n+k],&y[n+k],&e_up(n+k,0),
			   &p(n+k,0),&ld,N,n+k,1,&ld)) break;
	}  /* if column_regular */
  ...
  update_x_from_Dy_schur(work,lwork,&L(0,0),y,x,N,reg_index,reg_pos,&ld);
