...
#include "lss/nc_halo_density_profile_mean.h"
...
G_DEFINE_TYPE (NcHaloDensityProfileMean, nc_halo_density_profile_mean, NC_TYPE_HALO_DENSITY_PROFILE);

#define VECTOR  (NCM_MODEL (dpmean)->params)
...
static void
nc_halo_density_profile_Mean_init (NcHaloDensityProfileMean *dpmean)
{
}

static void
_nc_halo_density_profile_mean_set_property (GObject *object, guint prop_id, const GValue *value, GParamSpec *pspec)
{
  /* NcHaloDensityProfileNFW *dpnfw = NC_HALO_DENSITY_PROFILE_NFW (object); */
  g_return_if_fail (NC_IS_HALO_DENSITY_PROFILE_MEAN (object));
...
static void
_nc_halo_density_profile_mean_get_property (GObject *object, guint prop_id, GValue *value, GParamSpec *pspec)
{
  /* NcHaloDensityProfileNFW *dpnfw = NC_HALO_DENSITY_PROFILE_NFW (object); */
  g_return_if_fail (NC_IS_HALO_DENSITY_PROFILE_MEAN (object));
...
static void
_nc_halo_density_profile_mean_finalize (GObject *object)
{
  /* Chain up : end */
  G_OBJECT_CLASS (nc_halo_density_profile_mean_parent_class)->finalize (object);
}

static gdouble _nc_halo_density_profile_mean_eval_outer_dl_density (NcHaloDensityProfile *dp, const gdouble x);
static gdouble _nc_halo_density_profile_mean_eval_outer_dl_2d_density (NcHaloDensityProfile *dp, const gdouble X);

static void
nc_halo_density_profile_mean_class_init (NcHaloDensityProfileMeanClass *klass)
{
  GObjectClass *object_class          = G_OBJECT_CLASS (klass);
  NcHaloDensityProfileClass *dp_class = NC_HALO_DENSITY_PROFILE_CLASS (klass);
  NcmModelClass *model_class          = NCM_MODEL_CLASS (klass);
  
  model_class->set_property = &_nc_halo_density_profile_mean_set_property;
  model_class->get_property = &_nc_halo_density_profile_mean_get_property;
  object_class->finalize    = &_nc_halo_density_profile_mean_finalize;
  
  ncm_model_class_set_name_nick (model_class, "Mean Density Profile", "Mean");
  ncm_model_class_add_params (model_class, 0, 0, PROP_SIZE);
  
  /* Check for errors in parameters initialization */
  ncm_model_class_check_params_info (model_class);
  
  dp_class->eval_outer_dl_density    = &_nc_halo_density_profile_mean_eval_outer_dl_density;
  dp_class->eval_outer_dl_2d_density = &_nc_halo_density_profile_mean_eval_outer_dl_2d_density;
}

static gdouble
_nc_halo_density_profile_mean_eval_outer_dl_density (NcHaloDensityProfile *dp, const gdouble x)
{
/*  np.ones((len(r)), float) * self.rho_m 
* somehow here i have to get the mean density of the universe at z, and substitute on x
*/
  return 1.0 * x;
}
...
static gdouble
_nc_halo_density_profile_mean_eval_outer_dl_2d_density (NcHaloDensityProfile *dp, const gdouble X)
{
/*  TODO put a warning here, 2d density cannot be calculated with mean density */
  return 0.0 ;
}
 ...
 */
void 
nc_halo_density_profile_mean_class_set_ni (gboolean num)
{
  NcHaloDensityProfileClass *dp_class        = g_type_class_ref (NC_TYPE_HALO_DENSITY_PROFILE_MEAN);
  NcHaloDensityProfileClass *dp_parent_class = g_type_class_peek_parent (dp_class);
  
  if (num)
  {
    dp_class->eval_outer_dl_2d_density = dp_parent_class->eval_outer_dl_2d_density;
  }
  else
  {
    dp_class->eval_outer_dl_2d_density = &_nc_halo_density_profile_mean_eval_outer_dl_2d_density;
 ...
 */
NcHaloDensityProfileMean *
nc_halo_density_profile_mean_new (const gdouble z)
{
  NcHaloDensityProfileMean *dp_mean = g_object_new (NC_TYPE_HALO_DENSITY_PROFILE_MEAN,
                                                  "z", z,
                                                  NULL);
