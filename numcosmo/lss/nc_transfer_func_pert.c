...
static void
_nc_transfer_func_pert_prepare (NcTransferFunc *tf, NcHIReion *reion, NcHICosmo *cosmo)
{
	NcTransferFuncPert *tf_pert = NC_TRANSFER_FUNC_PERT (tf);
	if (!tf_pert->init)
	{
		NcRecomb *recomb = nc_recomb_new_from_name ("NcRecombSeager");
		tf_pert->pert = nc_pert_linear_new (cosmo, reion, recomb, 1 << 3, 1e-7, 1e-7, 1e-10, 1e-10);
