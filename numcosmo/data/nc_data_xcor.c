...
nc_data_xcor_dispose (GObject *object)
{
  ...
  /* Chain up : end */
  G_OBJECT_CLASS (nc_data_xcor_parent_class)->dispose (object);
}
...
_nc_data_xcor_prepare (NcmData *data, NcmMSet *mset)
{
   ...
   */
  gboolean prep[NC_DATA_XCOR_MAX][NC_DATA_XCOR_MAX] = {
    {FALSE}
  };
  ...
  for (a = 0; a < nobs; a++)
  {
    ...
    if (prep[a][a])
    {
      ...
      nc_xcor_general (dxc->xc, xcl1, NULL, cosmo, 0, dxc->xcab[a][a]->ell_th_cut_off, cl_th_0_aa);
    ...
    if (nobs > 1)
    {
      for (b = a + 1; b < nobs; b++)
      {
        if (prep[a][b])
        {
          ...
          nc_xcor_general (dxc->xc, xcl1, xcl2, cosmo, 0, dxc->xcab[a][b]->ell_th_cut_off, cl_th_0_ab);
