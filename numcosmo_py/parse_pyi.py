...
def _search_overridden_symbols(local_input: str) -> list[str]:
    symbols: list[str] = []
    parents: list[str] = []
    ...
    for i, line in enumerate(local_input.splitlines()):
...
def parse(local_input: str) -> ParseResult:
    """Parse pyi files."""
    overridden_symbols = _search_overridden_symbols(local_input)
    root = ast.parse(local_input)
