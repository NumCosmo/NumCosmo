...
Ncm.cfg_init()
d = int(sys.argv[1]); nw = int(sys.argv[2]); niter = int(sys.argv[3])
kname = sys.argv[4]; h = float(sys.argv[5]); shrink = sys.argv[6] == "1"
lf = float(sys.argv[7]); interp = sys.argv[8] != "0"
rwprob = float(sys.argv[9]) if len(sys.argv) > 9 else None
...
S = Q @ np.diag(ev) @ Q.T; L = np.linalg.cholesky(S); Sinv = np.linalg.inv(S)
m2lnp = lambda X: np.einsum("ij,jk,ik->i", X, Sinv, X)

apes = APES(nwalkers=nw, ndim=d, model=None, log_prob=lambda t, _: -0.5 * np.asarray(t) @ Sinv @ np.asarray(t),
...
if rwprob is not None:
    walker.set_random_walk_prob(rwprob)
...
def score(sd, X):
    ...
    n = len(X)
    r_x = -0.5 * (np.array([sd.eval_m2lnp(Ncm.Vector.new_array(x.tolist())) for x in X]) - m2lnp(X))
    v = Ncm.Vector.new(d)
    Y = np.empty((n, d))
    for i in range(n):
        sd.sample(v, NRNG)
...
print(f"d={d} {kname} h={h} shrink={int(shrink)} interp={int(interp)} rwprob={walker.get_random_walk_prob():.3f}", flush=True)
print(f"  chain acceptance            = {chain_acc:.3f}", flush=True)
print(f"  walker sd0, x = first half  = {score(sd0, ens[:half]):.3f}   (nk={sd0.get_n_kernels()})", flush=True)
print(f"  walker sd1, x = second half = {score(sd1, ens[half:]):.3f}   (nk={sd1.get_n_kernels()})", flush=True)
