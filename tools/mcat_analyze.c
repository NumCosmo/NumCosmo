...
main (gint argc, gchar *argv[])
{
  ...
  GOptionEntry entries[] =
  {
    ...
    { "accept-ratio",   'A', 0, G_OPTION_ARG_NONE,         &accept_ratio,     "Print the acceptance ratio evolution.", NULL },
  ...
  else
  {
    ...
    if (accept_ratio)
    {
      ...
      for (i = 1; i < last_t; i++)
      {
        const gdouble ar = g_array_index (accept_ratio_array, gdouble, i - 1);

        ncm_message ("%10u % 22.15g\n", i, ar);
      }
    }
