...
Implements a primordial cosmology model and computes its power spectrum.
...
Import the required modules and initialize the library with `Ncm.cfg_init()`.
...
class that inherits from the [[numcosmo|NcHIPrim]] abstract class. This process involves using the
[[numcosmo-math|NcmModelBuilder]] class to construct the model and register it within the GObject type
system. Below, we outline the steps to achieve this.

### Step 1: Define the Model Using [[numcosmo-math|NcmModelBuilder]]

We start by creating a new [[numcosmo-math|NcmModelBuilder]] object, which allows us to define a new
model (`NcHIPrimExample`) that implements the [[numcosmo|NcHIPrim]] abstract class. The
[[numcosmo-math|NcmModelBuilder]] class provides a convenient way to define models and their parameters.
...
Add the parameters used by the primordial spectrum:
...
`NcHIPrimExample` class, which is now a subclass of [[numcosmo|NcHIPrim]] with the specified
parameters.
...
At this point, `HIPrimExample` is a fully defined subclass of [[numcosmo|NcHIPrim]] with the
parameters `A_s`, `n_s`, `a`, `b`, and `c`. However, to make the model functional, we
need to implement the [[numcosmo|NcHIPrim]] interface by subclassing `HIPrimExample` and providing
the necessary methods.

### Implementing the [[numcosmo|NcHIPrim]] Interface

To complete the model, we need to subclass `HIPrimExample` and implement the methods
required by the [[numcosmo|NcHIPrim]] interface. This involves defining how the primordial power
...
The `NcHIPrimExample` model is ready for use.

----------------------------------------------------------------------------------------

## Computing the Power Spectrum

Compute the `HIPrimExampleImpl` power spectrum and compare it with the
unmodified spectrum.
...
# attaching the reionization and primordial spectrum submodels at
# construction time
