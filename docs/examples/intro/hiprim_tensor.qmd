...
date: "2023-05-22"
abstract: |
  Computes TT, TE, EE, and BB spectra with tensor modes.
...
Tests tensor modes and computes TT, TE, EE, and BB spectra.
...
Import the modules and call `Ncm.cfg_init()`.
...
Create an `Nc.HIPrimPowerLaw` model and set `r` and `n_T`.
...
We create a new homogeneous and isotropic cosmological model ([[numcosmo|NcHICosmoDEXcdm]]) and add the primordial and reionization submodels.

```{python}
# Create a new reionization object
reion = Nc.HIReionCamb.new()

# Create a new cosmological model, attaching the reionization and
# primordial submodels at construction time
...
The example computes and plots TT, TE, EE, and BB spectra with scalar and
