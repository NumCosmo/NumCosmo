...
Inspects NumCosmo data objects through the `numcosmo_py`[^numcosmo_py]
package.

A Python companion script is provided as `inspect_data_objects.py`.

The example lists data-object source files, inspects one source file, and
creates a `DataHubble` object to inspect its runtime properties.
...
def find_numcosmo_data_dir():
    ...
    This function supports both the current source-tree layout:

        numcosmo/nc/data/

    and the older layout:

        numcosmo/data/

    It also accepts NUMCOSMO_SOURCE_DIR pointing directly to the data directory.
    """
    ...
    relative_data_dirs = [
        Path("numcosmo") / "nc" / "data",
        Path("numcosmo") / "data",
    ]

    for candidate in candidates:
        # Allow NUMCOSMO_SOURCE_DIR to point directly to the data directory.
        if candidate.is_dir() and any(candidate.glob("nc_data*.h")):
            return candidate

        # Otherwise treat the candidate as a possible repository root.
        for relative_data_dir in relative_data_dirs:
            data_dir = candidate / relative_data_dir
...
if data_dir is None:
    ...
    print("or:")
    print("  export NUMCOSMO_SOURCE_DIR=$HOME/dev/NumCosmo/numcosmo/nc/data")
else:
