...
This tutorial defines an abstract curve model in C and fits concrete
implementations.

NumCosmo separates an abstract model, which declares only the fundamental
quantities of a phenomenon, from the implementations that choose a functional
form and a parametrization for them. The separation gives four things:

- Analysis code can be written against the abstract interface, without
  reference to any particular implementation.
- The fundamental quantities of the model stay distinct from the choices made
  when implementing them.
- An implementation can be changed or replaced without affecting the rest of
  the codebase.
- Separate groups can develop implementations independently.

Models can also be written in Python; that is covered in a separate tutorial.


## Our abstract model

The phenomenon here is a curve $f(x)$. Its only fundamental quantities are the
curve itself and its domain $[x_l,\;x_u]$: no functional form and no
parametrization, since those belong to the implementations.

See the [GObject tutorial](gobject.qmd) for the object framework.

The abstract `NcCurve` type subclasses [[numcosmo-math|NcmModel]].
`nc_curve_f` evaluates $f(x)$; `xl` and `xu` define the domain.

## Abstract model header

The header defines the virtual method, class structure, properties, and
...
The [[numcosmo-math|NcmModel]] framework is designed to support 
...
[[numcosmo-math|NcmMSet]] (model set) object. We use the [[numcosmo-math|NCM_MSET_MODEL_DECLARE_ID]]
macro to declare the id function `nc_curve_id` in the [[numcosmo-math|NcmMSet]]
framework.
```C
...
be instantiated) that is a child of [[numcosmo-math|NcmModel]].
```C
...
The [[numcosmo-math|NcmMSet]] framework provides the macro 
...
in the [[numcosmo-math|NcmMSet]] type list.
...
as the set/get functions. Since we are using the [[numcosmo-math|NcmModel]]
...
#### Registering the ID in [[numcosmo-math|NcmMSet]]
...
type in the [[numcosmo-math|NcmMSet]] type list. This is
necessary to set/get objects of this type from a [[numcosmo-math|NcmMSet]].
...
We use the [[numcosmo-math|NCM_MODEL_SET_IMPL_FUNC]] to generate the code for the implementation
