...
Computes adiabatic and entropy perturbation spectra for a two-component
radiation-plus-fluid bouncing cosmology.
...
$$
with $i=\mathrm{r},w$ for radiation and the $w$-fluid. The [[numcosmo|NcHIPertITwoFluids]] interface (implemented by `HICosmoQGRW`) returns these weights in code units. In these units one finds
$$
...
# | fig-cap: >
# |   Gravitational weights $\gamma_\mathrm{r}$, $\gamma_w$, and $F_\nu$. Shaded
# |   areas show matter/radiation dominance; vertical lines mark $x_{\rm eq}$ and
# |   $x=1$.
...
# | fig-cap: >
# |   Adiabatic truncation estimates for each mode (compare with $c_iF_\nu$). Shaded
# |   regions: matter/radiation dominance; vertical lines mark $x_{\rm eq}$ and $x=1$.
# | fig-cap-location: bottom
# | code-fold: true
...
# | fig-cap: >
# |   Zoom around matter-radiation equality showing adiabatic truncation estimates.
# | fig-cap-location: bottom
# | code-fold: true
...
# | fig-cap: >
# |   Mode evolution for $\zeta_i$; colors show comoving wavelength $\lambda$ (Mpc).
# |   Top: $\lambda$ vs. $c_i x R_H$. Bottom: $k^{-1}\zeta_i$ evolution; vertical
# |   lines mark sound-horizon crossings.
# | fig-cap-location: bottom
# | code-fold: true
...
# | fig-cap: >
# |   Mode evolution for the entropy perturbations $\Delta\zeta_i$; color bar shows
# |   $\lambda$. Top: $\lambda$ vs. $c_i x R_H$. Bottom: $k^{-1}\Delta\zeta_i$
# |   evolution.
# | fig-cap-location: bottom
# | code-fold: true
...
# | fig-cap: >
# |   Mode evolution for density perturbations $\delta_{\rho i}$; color bar shows
# |   $\lambda$. Top: $\lambda$ vs. $c_i x R_H$. Bottom: $k^{-1}\bar{\delta}_{\rho
# |   i}$ evolution.
# | fig-cap-location: bottom
# | code-fold: true
...
# | fig-cap: >
# |   Mode evolution for $\Delta\delta_{\rho i}$; color bar shows $\lambda$. Top:
# |   $\lambda$ vs. $c_i x R_H$. Bottom: $k^{-1}\Delta\delta_{\rho i}$ evolution.
# | fig-cap-location: bottom
# | code-fold: true
...
# | fig-cap: >
# |   Mode evolution for momentum $\Pi_{\zeta i}$; color bar shows $\lambda$. Top:
# |   $\lambda$ vs. $c_i x R_H$. Bottom: $k^{-1}\Pi_{\zeta i}$ evolution.
# | fig-cap-location: bottom
# | code-fold: true
...
# | fig-cap: >
# |   Dimensionless power $\mathcal{P}_{\zeta_i}(\alpha)$ for modes 1 (left) and 2
# |   (right); colors show $\lambda$ (Mpc).
# | fig-cap-location: bottom
# | code-fold: true
...
# | fig-cap: >
# |   Time evolution of $\mathcal{P}_{\Pi_{\zeta i}}(\alpha)$ for each mode; colors
# |   show $\lambda$ (Mpc).
# | fig-cap-location: bottom
# | code-fold: true
...
# | fig-cap: >
# |   Time evolution of $\mathcal{P}_{\Delta\zeta_i}(\alpha)$ for each mode; colors
# |   show $\lambda$ (Mpc).
# | fig-cap-location: bottom
# | code-fold: true
...
# | fig-cap: >
# |   Total adiabatic power $\mathcal{P}_{\zeta}(\alpha)$ and entropy power
# |   $\mathcal{P}_{\Delta\zeta}(\alpha)$; colors show $\lambda$.
# | fig-cap-location: bottom
# | code-fold: true
...
# | fig-cap: >
# |   Time evolution of $\mathcal{P}_{\delta_{\rho i}}(\alpha)$ for each mode;
# |   colors show comoving wavelength $\lambda$ (Mpc).
# | fig-cap-location: bottom
# | code-fold: true
...
# | fig-cap: >
# |   Time evolution of $\mathcal{P}_{\Delta\delta_{\rho i}}(\alpha)$ for each
# |   mode; colors show comoving wavelength $\lambda$ (Mpc).
# | fig-cap-location: bottom
# | code-fold: true
...
# | fig-cap:
# | - >
# |   Total density power $\mathcal{P}_{\delta_{\rho}}(\alpha)$; colors show
# |   comoving wavelength $\lambda$ (Mpc).
# | - >
# |   Difference density power $\mathcal{P}_{\Delta\delta_{\rho}}(\alpha)$; colors
# |   show $\lambda$ (Mpc).
# | fig-cap-location: bottom
# | layout-ncol: 2
# | code-fold: true
...
# | fig-cap: >
# |   Dimensionless power $\mathcal{P}_{\zeta}(k)$ at selected times; vertical lines
# |   mark characteristic wavenumbers.
# | fig-cap-location: bottom
# | layout-ncol: 2
# | code-fold: true
...
# | fig-cap: >
# |   CMB-range dimensionless power $\mathcal{P}_{\zeta}(k)$ with linear and quadratic
# |   fits (pivot 0.05 Mpc$^{-1}$).
# | fig-cap-location: bottom
# | code-fold: true
...
# | fig-cap: >
# |   Dimensionless power $\mathcal{P}_{\Pi_\zeta}(k)$ at selected times; vertical
# |   lines mark characteristic wavenumbers.
# | fig-cap-location: bottom
# | layout-ncol: 2
# | code-fold: true
...
# | fig-cap: >
# |   Dimensionless power $\mathcal{P}_{\delta_{\rho}}(k)$ and
# |   $\mathcal{P}_{\Delta\delta_{\rho}}(k)$ at selected times; solid/dashed lines
# |   show mode 1/2.
# | fig-cap-location: bottom
# | layout-ncol: 2
# | code-fold: true
...
# | fig-cap:
# | - >
# |   Dimensionless power $\mathcal{P}_{\delta_{\mathrm{r}}}(k)$ (solid) at selected
# |   times; vertical lines mark characteristic wavenumbers.
# | - >
# |   Dimensionless power $\mathcal{P}_{\delta_w}(k)$ (dashed) at selected times;
# |   same layout.
# | fig-cap-location: bottom
# | layout-ncol: 2
# | code-fold: true
...
# | fig-cap: >
# |   Dimensionless power of fluid velocity potentials $\mathcal{P}_{\mathcal{V}_r}$
# |   (solid) and $\mathcal{P}_{\mathcal{V}_w}$ vs. $k$.
# | fig-cap-location: bottom
# | code-fold: true
...
# | fig-cap: >
# |   Evolution of the tensor power spectrum $\mathcal{P}_h(\alpha)$ for each mode.
# | fig-cap-location: bottom
# | code-fold: true
...
# | fig-cap:
# | - >
# |   Tensor power $\mathcal{P}_h(k)$ at an early time; vertical lines mark
...
for i, alpha in enumerate(alpha_a):
    ...
    x_latex = latex_float(cosmo_rw.x_alpha(alpha), precision=4)
    print(f"alpha = {alpha:.2f}, x = {x_latex}: mean r in CMB scales = {r_cmb_mean:.4e}")

```


## Preparing the fiducial power spectrum
...
print(f"  Deviation from expected slope: {abs(coeffs[0] + 2.5):.6f}")
```

Now we can visualize this scaling relationship:

```{python}
# | label: fig-pk0-scaling
# | fig-cap: >
# |   Scaling of the power spectrum normalization $\bar{\mathcal{P}}(10^{-3}; w)$ with
# |   the equation-of-state parameter $w$. The comparison with the expected $w^{-5/2}$
# |   power law shows excellent agreement.
# | fig-cap-location: bottom
# | code-fold: true
...
We can also examine the normalized power spectrum by dividing out the expected scaling factor:

```{python}
# | label: fig-pk0-normalized
# | fig-cap: >
# |   Normalized power spectrum $\bar{\mathcal{P}}(10^{-3}; w)$ divided by the scaling
# |   factor $\ell_\textsc{P}^2x_b^2 \Omega_w^2 / (R_{H}^2 \Omega_r c_w^5)$, where
# |   $c_w = \sqrt{w}$. This removes the dominant $w$ dependence.
# | fig-cap-location: bottom
# | code-fold: true
...
```{python}
# | label: fig-cmb-cls-comparison
# | fig-cap: >
# |   CMB temperature power spectra comparison. Top: $\mathcal{D}_\ell =
# |   \ell(\ell+1)C_\ell/(2\pi)$ for two-fluids (red) and power-law (blue dashed).
# |   Bottom: Ratio $\mathcal{D}_\ell^{\rm 2F}/\mathcal{D}_\ell^{\rm PL}$.
# | fig-cap-location: bottom
# | code-fold: true
...
def compute_D_ell_twofluid(lmax=2500):
    ...
    cosmo = Nc.HICosmoDEXcdm(reion=reion, prim=hiprim)
    cosmo.cmb_params()
...
print(f"  Two-fluids peak: {Dell_2f.max():.2f} uK^2 at ell = {ell[Dell_2f.argmax()]}")
print(f"  Power-law peak:  {Dell_pl.max():.2f} uK^2 at ell = {ell[Dell_pl.argmax()]}")
