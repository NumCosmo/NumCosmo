...
alpha_mode_end = -30.0  # Final time in alpha for the mode evolution
...
# | fig-cap: Gravitational weights $\gamma_\mathrm{r}$ and $\gamma_w$, and $F_\nu$ (ratio of Hubble radius to physical wavelength). Shaded regions indicate matter and radiation dominance.
...
# Vertical markers
ax.axvline(x=alpha_eq, color="k", linestyle="dashed", lw=lw, label=r"$x_\mathrm{eq}$")
ax.axvline(x=alpha0, color="k", linestyle="dotted", lw=lw, label=r"$x=1$")

ax.axvspan(alpha_mode_ini, alpha_eq, **md, label="r")
ax.axvspan(alpha_eq, alpha_mode_end, **rd, label="w")
...
For the adiabatic approximation to hold, the adiabatic correction factors for each mode variable, $\zeta$, $Q$, and their conjugate momenta $P_\zeta$ and $P_Q$, must remain smaller than unity. Although these correction terms can be complex, their behavior is primarily governed by the factor $1/(c_i F_\nu)$. In practice, evaluating $c_i F_\nu \gg 1$ and $c_i F_\nu \ll 1$ delineates the sub-Hubble and super-Hubble regimes, respectively. The transition at $c_i F_\nu = 1$ marks the end of the adiabatic regime.
...
# | fig-cap: The adiabatic truncation error for each mode and its square root. Shaded regions indicate matter and radiation dominance.
# | fig-cap-location: bottom
# | code-fold: true
...
scale_array = [
    (
        delta_zeta1,
        delta_zeta2,
        r"$\Delta_{\zeta_1} c_1 F_\nu$",
        r"$\Delta_{\zeta_2} c_2 F_\nu$",
    ),
    (delta_Q1, delta_Q2, r"$\Delta_{Q_1} c_1 F_\nu$", r"$\Delta_{Q_2} c_2 F_\nu$"),
    (
        delta_Pzeta1,
        delta_Pzeta2,
        r"$\Delta_{P_{\zeta_1}} c_1 F_\nu$",
        r"$\Delta_{P_{\zeta_2}} c_2 F_\nu$",
    ),
    (
        delta_PQ1,
        delta_PQ2,
        r"$\Delta_{P_{Q_1}} c_1 F_\nu$",
        r"$\Delta_{P_{Q_2}} c_2 F_\nu$",
    ),
]
...
# | fig-cap: Zoom-in of the adiabatic truncation error for each mode around matter-radiation equality. Shaded regions indicate matter and radiation dominance.
# | fig-cap-location: bottom
# | code-fold: true
...
# Vertical markers
ax1.axvline(x=alpha_eq, color="k", linestyle="dashed", lw=lw, label=r"$x_\mathrm{eq}$")
ax2.axvline(x=alpha_eq, color="k", linestyle="dashed", lw=lw, label=r"$x_\mathrm{eq}$")
...
# Define the modes to compute and prepare a color map to depict them
k_values = np.geomspace(1.0e-6, 1.0e6, 6)
...
for k, modes in zip(k_values, modes_list):
    lambda_com = 1.0 / k
    color = cmap(norm(lambda_com))
...
all power spectra for the observables of interest, which are stored in the `power_spectra_list` variable.
...
# | fig-cap: "Power spectrum of the entropy perturbation $\\Delta\\zeta_i$ for each mode, shown in a distinct color with comoving wavelength $\\lambda$ indicated in the color bar (in Mpc). The top panel shows the dimensionless power spectrum $k^3 P_{\\Delta\\zeta_i} / (2 \\pi^2)$, where $P_{\\Delta\\zeta_i}$ is the power spectrum of the entropy perturbation $\\Delta\\zeta_i = (\\zeta_{\\mathrm{r}i} - \\zeta_{w i})$. The extra factor $k^{-1}$ separates the modes visually. The vertical lines mark the times when $\\lambda = c_i x R_H$, corresponding to sound-horizon crossing ($c_i F_\\nu = 1$), and help identify the transition between sub- and super-Hubble behavior for each mode. The shaded regions indicate the radiation-dominated era (left) and the matter-dominated era (right)."
# | fig-cap-location: bottom
# | code-fold: true
...
k_for_pk_a = np.geomspace(1.0e-6, 1.0e6, 100)
k_for_plot_a = np.geomspace(1.0e-6, 1.0e6, 6000)

s_interp = pert.compute_spectrum(cosmo_rw, alpha_pk_end, k_for_pk_a * RH_Mpc, None)

power_spectra_k = {
    name: np.abs(
        [
            s_interp.eval(cosmo_rw, k * RH_Mpc).eval_obs(
                Nc.HIPertITwoFluidsObsMode.BOTH, obs, obs
            )
...
# | fig-cap: "Power spectrum of the adiabatic perturbation $\\zeta$ as a function of comoving wavenumber $k$ (in Mpc$^{-1}$). The plot shows the dimensionless power spectrum $k^3 P_{\\zeta} / (2 \\pi^2)$, where $P_{\\zeta}$ is the power spectrum of $\\zeta$. The vertical lines mark the times when $\\lambda = c_i x R_H$, corresponding to sound-horizon crossing ($c_i F_\\nu = 1$), and help identify the transition between sub- and super-Hubble behavior for each mode."
# | fig-cap-location: bottom
# | code-fold: true
...
# | fig-cap: "Power spectrum of the density perturbation $\\delta_{\\rho}$ as a function of comoving wavenumber $k$ (in Mpc$^{-1}$). The plot shows the dimensionless power spectrum $k^3 P_{\\delta_{\\rho}} / (2 \\pi^2)$, where $P_{\\delta_{\\rho}}$ is the power spectrum of $\\delta_{\\rho}$. The vertical lines mark the times when $\\lambda = c_i x R_H$, corresponding to sound-horizon crossing ($c_i F_\\nu = 1$), and help identify the transition between sub- and super-Hubble behavior for each mode."
# | fig-cap-location: bottom
# | code-fold: true
...
# | fig-cap: "Power spectrum of the individual fluid density perturbations $\\delta_{\\mathrm{r}}$ (solid) and $\\delta_{w}$ (dashed) as a function of comoving wavenumber $k$ (in Mpc$^{-1}$). The plot shows the dimensionless power spectra $k^3 P_{\\delta_{\\mathrm{r}}} / (2 \\pi^2)$ and $k^3 P_{\\delta_{w}} / (2 \\pi^2)$, where $P_{\\delta_{\\mathrm{r}}}$ and $P_{\\delta_{w}}$ are the power spectra of $\\delta_{\\mathrm{r}}$ and $\\delta_{w}$, respectively. The vertical lines mark the times when $\\lambda = c_i x R_H$, corresponding to sound-horizon crossing ($c_i F_\\nu = 1$), and help identify the transition between sub- and super-Hubble behavior for each mode."
# | fig-cap-location: bottom
# | code-fold: true
...
fig.savefig(
    os.path.join(fig_dir, "delta_rw_power_spectrum_k.pdf"), bbox_inches="tight"
)
fig.set_size_inches(6, 3)
plt.show()
```
...
# | fig-cap: "Power spectrum of the fluid velocity potentials $\\mathcal{V}_{\\mathrm{r}}$ (solid) and $\\mathcal{V}_{w}$ (dashed) as a function of comoving wavenumber $k$ (in Mpc$^{-1}$). The plot shows the dimensionless power spectra $k^3 P_{\\mathcal{V}_{\\mathrm{r}}} / (2 \\pi^2)$ and $k^3 P_{\\mathcal{V}_{w}} / (2 \\pi^2)$, where $P_{\\mathcal{V}_{\\mathrm{r}}}$ and $P_{\\mathcal{V}_{w}}$ are the power spectra of $\\mathcal{V}_{\\mathrm{r}}$ and $\\mathcal{V}_{w}$, respectively. The vertical lines mark the times when $\\lambda = c_i x R_H$, corresponding to sound-horizon crossing ($c_i F_\\nu = 1$), and help identify the transition between sub- and super-Hubble behavior for each mode."
# | fig-cap-location: bottom
# | code-fold: true
...
fig.savefig(
    os.path.join(fig_dir, "fku_rw_power_spectrum_k.pdf"), bbox_inches="tight"
)
fig.set_size_inches(6, 3)
plt.show()
```
