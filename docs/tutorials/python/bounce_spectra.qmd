...
for i, alpha in enumerate(alpha_a):
    ...
    x_latex = latex_float(cosmo_rw.x_alpha(alpha), precision=4)
    print(f"α = {alpha:.2f}, x = {x_latex}: mean r in CMB scales = {r_cmb_mean:.4e}")
...
# | fig-cap: "Scaling of the power spectrum normalization $\\bar{\\mathcal{P}}(10^{-3}; w)$ with the equation-of-state parameter $w$. The comparison with the expected $w^{-5/2}$ power law shows excellent agreement."
# | fig-cap-location: bottom
# | code-fold: true
...
# | fig-cap: "Normalized power spectrum $\\bar{\\mathcal{P}}(10^{-3}; w)$ divided by the scaling factor $\\ell_\textsc{P}^2x_b^2 \\Omega_w^2 / (R_{H}^2 \\Omega_r c_w^5)$, where $c_w = \\sqrt{w}$. This removes the dominant $w$ dependence."
# | fig-cap-location: bottom
# | code-fold: true
...
# | fig-cap: "CMB temperature power spectra comparison. Top: $\\mathcal{D}_\\ell = \\ell(\\ell+1)C_\\ell/(2\\pi)$ for two-fluids (red) and power-law (blue dashed). Bottom: Ratio $\\mathcal{D}_\\ell^{\\rm 2F}/\\mathcal{D}_\\ell^{\\rm PL}$."
# | fig-cap-location: bottom
# | code-fold: true
