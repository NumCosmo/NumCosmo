...
Generates mock cluster weak-lensing data with varying cluster positions and
masses.
...
Define an NFW halo profile, a cluster-position model, and an
`Nc.WLSurfaceMassDensity` calculator.
...
p_dist = Nc.GalaxyPositionFactorFlat.new(
    galaxies_ra0, galaxies_ra1, galaxies_dec0, galaxies_dec1
)
z_dist = Nc.GalaxyRedshiftFactorComposed.new(0.1, 2.0)
z_pop = Nc.GalaxyRedshiftPopLSSTSRD.new_y1_source()
z_obs_sel = Nc.GalaxyRedshiftObsGauss.new()

s_dist = Nc.GalaxyShapeFactorVarAdd.new(ellip_conv)
s_pop = Nc.GalaxyShapePopGauss.new()

s_pop["sigma"] = galaxy_shape_e_rms
...
The `Nc.DataClusterWLFactor` object is used to create a new cluster model, specifying the distribution of the source galaxies, the redshift distribution, and the position distribution. 
...
```{python}
min_r, max_r = 0.3, 3.0
cluster = Nc.DataClusterWLFactor.new(p_dist, z_dist, s_dist)
...
To generate mock weak lensing data, we use the `Nc.DataClusterWLFactor` object created earlier. 
...
pos_data = Nc.GalaxyPositionFactorData.new(p_dist, mset)
z_data = Nc.GalaxyRedshiftFactorData.new(z_dist, mset)
s_data = Nc.GalaxyShapeFactorData.new(s_dist, mset, pos_data, z_data)
...
for i in range(n_galaxies):
    ...
    c1 = rng.gaussian_gen(0.0, 0.005)
    c2 = rng.gaussian_gen(0.0, 0.005)
    m = rng.gaussian_gen(0.1, 0.05)

    obs.set(Nc.GALAXY_REDSHIFT_OBS_GAUSS_COL_SIGMA0, i, sigma0)
    obs.set("std_noise", i, galaxy_shape_e_sigma)
    obs.set("c1", i, c1)
    obs.set("c2", i, c2)
    obs.set("m", i, m)
...
# | fig-cap: >
# |   Galaxy distribution around the cluster. The blue ellipses represent the observed
# |   galaxy shapes, while the red ellipses indicate the intrinsic galaxy shapes. The
# |   black dot marks the true cluster position.
...
def gen_galaxies(halo_ra: float, halo_dec: float, halo_mass: float) -> pd.DataFrame:
    ...
    for i in range(n_galaxies):
        ...
        std_noise = rng.uniform_gen(0.05, 0.25)
        c1 = rng.gaussian_gen(0.0, 0.005)
        c2 = rng.gaussian_gen(0.0, 0.005)
        m = rng.gaussian_gen(0.1, 0.05)
...
# | fig-cap: >
# |   Galaxy distribution around the cluster for different halo positions. The blue
# |   ellipses represent the observed galaxy shapes, while the black dot indicates the
# |   true halo position.
# | fig-cap-location: margin
# | fig-width: 100%
# | code-fold: true
...
# | fig-cap: >
# |   Galaxy distribution around the cluster for different halo masses. The blue
# |   ellipses represent the observed galaxy shapes. The black dot indicates the true
# |   halo position, with the size of the dot proportional to the halo mass.
# | fig-cap-location: margin
# | fig-width: 100%
# | code-fold: true
