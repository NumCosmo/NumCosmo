...
table = [
    ...
    + nc_cmp.compute_times(
        lambda: compute_using_ccl(models[0]["CCL"], z_a, nz_a, ells)
    ),
    ["Galaxy Weak Lensing", "NumCosmo"]
    + nc_cmp.compute_times(
        lambda: compute_using_nc(models[0]["NC"], dndz, ells, nc_wl_auto_v)
    ),
]
