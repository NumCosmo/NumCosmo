...
date: "2025-02-09"
abstract: |
    Compares NumCosmo and CCL cosmological background functions.
...
Compares distance, expansion, and growth functions from CCL and NumCosmo for
...
Compare total matter density, density parameters, and the normalized Hubble
function for each parameter set.
...
Use the CCL definition of $\Omega_m$ by setting `CCL_comp` to `True`.
...
Compare the distance observables implemented by CCL and NumCosmo.
...
Plot the distance comparisons for all parameter sets.
...
Measure the time required to compute cosmological distances with `timeit`.
...
The comparison covers background parameters, distances, growth, scale factor,
and critical surface mass density. Differences in growth increase with
redshift because CCL includes the full neutrino energy density.
